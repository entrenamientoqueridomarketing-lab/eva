...
function getFreedomState() {
  ...
  const accConsolidado = planConsolidatedCapital(planAcc);
  const opcConsolidado = planConsolidatedCapital(planOpc);
  const accPendiente = planPendingReleaseCapital(planAcc);
  const opcPendiente = planPendingReleaseCapital(planOpc);
  const accLibre = planReinvestableCapital(planAcc);
  const opcLibre = planReinvestableCapital(planOpc);
  const accOperativo = accConsolidado + accPendiente;
  const opcOperativo = opcConsolidado + opcPendiente;
...
function getDash() {
  ...
  // v26.2: el capital libre para operar ahora debe salir de los planes reales,
...
function effectivePlanInitialCapital(plan) {
  // v26.2: la Ruta de Libertad Financiera debe tomar el capital real del motor,
