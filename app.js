...
function cfg() {
  ...
  const planCapOpc = safeMoney(c.planCapitalOpciones, capOpc * usoOpc || 500) || capOpc * usoOpc || 500;
...
function dayUsage(plan, row) {
  const trades = tradesOnDate(plan.kind, row.date).filter(isCounted);
  const used = trades.reduce((s,t) => s + n(t.subtotal), 0);
  const openUsed = trades.filter(t => t.estado === 'Abierta').reduce((s,t) => s + n(t.subtotal), 0);
  const closedUsed = trades.filter(t => t.estado === 'Cerrada').reduce((s,t) => s + n(t.subtotal), 0);
  const real = trades.filter(t => t.estado === 'Cerrada').reduce((s,t) => s + n(t.pnl), 0);
  const mainTrade = trades[0] || null;
  return { trades, used, openUsed, closedUsed, real, mainTrade };
}
function statusForDay(plan, row) {
  const actualBase = actualCapitalBeforeDay(plan, row);
  const usage = dayUsage(plan, row);
  const goalBase = usage.used > 0 ? usage.used : actualBase;
  const minGoal = goalBase * plan.minPct;
  const targetGoal = goalBase * plan.targetPct;
  const real = usage.real;
  const remaining = Math.max(0, actualBase - usage.used);
  const nextTrade = usage.mainTrade ? Math.max(0, n(usage.mainTrade.subtotal) + (usage.mainTrade.estado === 'Cerrada' ? n(usage.mainTrade.pnl) : 0)) : actualBase;
  const nextTotal = Math.max(0, actualBase + real);
  ...
  return { actualBase, real, minGoal, targetGoal, status, tone, nextActual: nextTotal, remaining, used: usage.used, openUsed: usage.openUsed, closedUsed: usage.closedUsed, nextTrade, trades: usage.trades };
}
...
function renderPlan(kind) {
  ...
  return `
    ...
    <div class="kpi-grid">
      ...
      ${kpi('Capital disponible hoy', money(currStatus.actualBase), `Día ${day} de ${plan.diasMes}`, plan.isAcc?'teal':'blue')}
      ...
      ${kpi('Disponible después de cierre', money(currStatus.nextActual), currStatus.real ? `Resultado: ${money(currStatus.real)}` : `Disponible no usado: ${money(currStatus.remaining)}`, currStatus.real>=0?'teal':'red')}
    ...
    <div class="card">
      <div style="display:flex;justify-content:space-between;font-size:13px;margin-bottom:8px;gap:10px;flex-wrap:wrap">
        ...
        <span>Usado: <strong>${money(currStatus.used)}</strong> · Libre: <strong>${money(currStatus.remaining)}</strong> · Real: <strong class="${currStatus.real>=0?'green':'red'}">${money(currStatus.real)}</strong> / Meta: <strong>${money(currStatus.targetGoal)}</strong></span>
      </div>
    ...
    <div class="card">
      ...
      `<div class="tbl-wrap"><table>
        <thead><tr><th>Activo</th><th>Estado</th><th>Contabiliza</th><th>Fecha apertura</th><th>Fecha cierre</th><th>Inversión</th><th>P&L</th><th>P&L %</th><th>Siguiente trade</th><th>Libre</th><th></th></tr></thead>
        <tbody>${dayTrades.map(t => {
          ...
          return `<tr>
            ...
            <td>${money(Math.max(0, currStatus.actualBase - n(t.subtotal)))}</td>
    ...
    <div class="card">
      <div class="tbl-wrap"><table class="plan-day-table">
        <thead><tr><th>Día</th><th>Fecha</th><th>Disponible inicial</th><th>Usado</th><th>Libre</th><th>Mínimo</th><th>Meta ideal</th><th>Resultado real</th><th>Siguiente trade</th><th>Estado</th><th></th></tr></thead>
        <tbody>${daily.map(r => { const st = statusForDay(plan, r); return `<tr class="${r.day===day?'current-day':''}">
          ...
          <td><strong>${money(st.nextTrade)}</strong></td>
