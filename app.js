...
function unitTradesWithFallback(plan) {
  ...
  return all.map(t => {
    let unit;
    if (n(t.planGlobalUnit) > 0) unit = n(t.planGlobalUnit);
    else if (plan.isAcc) unit = planGlobalUnitForTrade(t, plan);
    else unit = ++optionIndex;
...
function renderPlan(kind) {
  ...
  const monthClosed = unitTradesWithFallback(plan)
    .filter(t => isCounted(t) && t.estado === 'Cerrada' && n(t._planUnitResolved) >= unitStart && n(t._planUnitResolved) <= unitEnd);
  ...
  return `
    ...
    <div class="card">
      <div class="tbl-wrap"><table class="plan-day-table">
        <thead><tr><th>${plan.unitLabel}</th><th>${plan.isAcc?'Fecha':'Fecha estimada'}</th><th>Inicio mín.</th><th>Después mín.</th><th>Inicio ideal</th><th>Después ideal</th><th>Real antes</th><th>Operable</th><th>Trades</th><th>Usado</th><th>Libre</th><th>Meta mín.</th><th>Meta ideal</th><th>Real</th><th>Real vs mín.</th><th>Real vs ideal</th><th>Estado</th><th></th></tr></thead>
        <tbody>${daily.map(r => { const st = statusForDay(plan, r); const diffIdeal = st.actualBase - r.start; const diffMin = st.actualBase - r.startMin; return `<tr class="${r.day===day?'current-day':''}">
          ...
          <td><strong>${st.tradeCount || 0}</strong></td>
          <td>${money(st.used)}</td>
