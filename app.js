...
//        El buying power capturado queda como corte/manual de cuenta.
...
function currentPlanFreeForNewEntries(plan) {
  ...
  try {
    ...
    // En opciones, si existe un capital actual/cuenta capturado y no hay posiciones abiertas,
    // ese dato manda sobre cualquier planCapital viejo inflado (ej. $1,100 manual).
    const accountCap = Math.max(0, n(plan.capCuenta));
    const occupied = planOccupiedCapital(plan);
    if (!plan.isAcc && accountCap > 0) {
      free = Math.max(0, accountCap - occupied);
    }
    return free;
  } catch(e) {
...
function getDash() {
  ...
  const capitalTotalEstrategico = disponibleReal + ocupado; // Capital libre en planes + capital dentro de posiciones abiertas.
...
function libertadSummaryCard(compact = false) {
  ...
  return `<div class="card">
    ...
    <div class="kpi-grid" style="margin-top:12px">
      ...
      ${kpi('Capital libre en planes', money(f.capitalLibrePlanes), `Acciones ${money(f.accLibre)} + Opciones ${money(f.opcLibre)}`, f.capitalLibrePlanes>0?'teal':'amber')}
...
function renderLibertad() {
  ...
  return `
    ...
    <div class="card">
      <div class="tbl-wrap"><table>
        <thead><tr><th>Motor</th><th>Capital consolidado</th><th>Pendiente de liberar</th><th>Capital operativo</th><th>Libre actual del plan</th></tr></thead>
        <tbody>
      ...
      <div class="alert-info" style="margin-top:10px">Estos valores salen de tus trades y planes. El buying power manual queda como corte de Charles; la Ruta usa el capital real calculado desde Acciones y Opciones.</div>
    </div>
