...
function getDash() {
  ...
  const capitalConsolidado = base + pnl;                  // Base del sistema: capital inicial/configurado + P&L cerrado.
  const capitalReinvertible = Math.max(0, capitalConsolidado - ocupado); // Efectivo no amarrado a posiciones abiertas.
  const valorTotalEstimado = capitalConsolidado + flotante; // Lo que valdría la cuenta si se marcara a mercado lo abierto.
  const comm = [...counted(D.acciones), ...counted(D.opciones)].reduce((s,x) => s + n(x.comision), 0);
  return {
    capAcc: c.capAcc, capOpc: c.capOpc,
    base, capitalConsolidado, capitalReinvertible, ocupado, ocupadoAcc, ocupadoOpc,
    valorTotalEstimado,
...
function renderDashboard() {
  ...
  return `
    ...
    <div class="alert-info"><strong>Lectura de capital:</strong> Capital reinvertible = dinero libre para volver a usar. Capital ocupado = dinero dentro de trades abiertos. Flotante = ganancia/pérdida abierta. Valor total estimado = capital consolidado + flotante.</div>
    ...
    <div class="kpi-grid">
      ${kpi('Capital consolidado', money(d.capitalConsolidado), `${mxn(d.capitalConsolidado)} · base + P&L cerrado`, 'blue')}
      ${kpi('Capital reinvertible', money(d.capitalReinvertible), 'Libre para nuevas entradas', d.capitalReinvertible>=0?'teal':'red')}
      ...
      ${kpi('Valor total estimado', money(d.valorTotalEstimado), `${mxn(d.valorTotalEstimado)} · consolidado + flotante`, d.valorTotalEstimado>=0?'teal':'red')}
...
function planConsolidatedCapital(plan, maxUnit = Infinity) {
  return Math.max(0, plan.planCapital + planRealizedPnl(plan, maxUnit));
}
...
function renderPlan(kind) {
  ...
  const capitalReinvertible = Math.max(0, capitalConsolidado - usedTotal);
  const valorTotalEstimado = capitalConsolidado + floatingTotal;
  ...
  return `
    ...
    <div class="kpi-grid compact-plan">
      ${kpi('Capital consolidado', money(capitalConsolidado), `${mxn(capitalConsolidado)} · mueve el compound`, 'blue')}
      ${kpi('Capital reinvertible', money(capitalReinvertible), 'Libre para volver a operar', capitalReinvertible>=0?'teal':'red')}
      ${kpi('Capital ocupado', money(usedTotal), `${openTrades.length} posición(es) abierta(s)`, usedTotal>0?'amber':'teal')}
      ${kpi('Ganancia/Pérdida flotante', money(floatingTotal), plan.isAcc ? 'Abiertas con precio actual' : 'Opciones sin cotización automática', floatingTotal>=0?'teal':'red')}
      ${kpi('Valor total estimado', money(valorTotalEstimado), 'Consolidado + flotante', valorTotalEstimado>=0?'teal':'red')}
