...
function cfg() {
  const c = D.config || {};
  const capAcc = n(c.capAcciones || c.capitalAcciones);
  const capOpc = n(c.capOpciones || c.capitalOpciones);
  const usoAcc = n(c.usoCapitalAcciones != null ? c.usoCapitalAcciones : 1) || 1;
  const usoOpc = n(c.usoCapitalOpciones != null ? c.usoCapitalOpciones : 0.33) || 0.33;
  const planCapAcc = n(c.planCapitalAcciones) || capAcc * usoAcc || 1000;
  const planCapOpc = n(c.planCapitalOpciones) || capOpc * usoOpc || 500;
  return {
    nombre: displayName(),
    capAcc,
    capOpc,
    metaAcc: n(c.metaAcciones || c.planMetaMaxAcciones || 0.03),
    metaOpc: n(c.metaOpciones || c.planMetaMaxOpciones || 0.20),
    metaMinAcc: n(c.planMetaMinAcciones || 0.01),
    metaMaxAcc: n(c.planMetaMaxAcciones || c.metaAcciones || 0.03),
    metaMinOpc: n(c.planMetaMinOpciones || 0.10),
    metaMaxOpc: n(c.planMetaMaxOpciones || c.metaOpciones || 0.20),
    planCapAcc,
    planCapOpc,
  ...
  return `
    ...
    <div class="plan-btn-grid">
      ...
      <div class="plan-big-btn options" onclick="showPage('plan-opciones')">
        ...
        <div class="plan-big-sub">Rango ${(cfg().metaMinOpc*100).toFixed(0)}%-${(cfg().metaMaxOpc*100).toFixed(0)}% · Plan ${money(cfg().planCapOpc)}</div>
      </div>
    </div>
...
function planSettings(kind) {
  ...
  return {
    ...
    diasMes: Math.max(1, c.diasMes || 20),
...
function buildMonthlyFromPlan(plan) {
  let cur = plan.planCapital;
  const rows = [];
  for (let m = 1; m <= plan.meses; m++) {
    const start = cur;
    cur = cur * Math.pow(1 + plan.targetPct, plan.diasMes);
    rows.push({ month: m, start, end: cur, gain: cur - start, pct: start ? (cur - start) / start : 0 });
  }
  return rows;
}
function buildDailyFromPlan(plan, month) {
  let projected = plan.planCapital * Math.pow(1 + plan.targetPct, (month - 1) * plan.diasMes);
  const rows = [];
  const offsetStart = (month - 1) * plan.diasMes;
  for (let d = 1; d <= plan.diasMes; d++) {
    const date = addPlanDays(plan.startDate, offsetStart + d - 1);
    const minTarget = projected * plan.minPct;
    const target = projected * plan.targetPct;
    const end = projected + target;
    rows.push({ day: d, globalDay: offsetStart + d, date, start: projected, minTarget, target, end });
    projected = end;
  }
  return rows;
}
...
function closedPlanTrades(kind) {
  return getPlanTrades(kind).filter(t => t.estado === 'Cerrada' && t.fechaCierre);
}
...
function pnlOnDate(kind, dateISO) {
  return closedPlanTrades(kind)
    .filter(t => t.fechaCierre === dateISO || tradeOpenDate(t, kind === 'acciones') === dateISO)
    .reduce((s,t) => s + n(t.pnl), 0);
}
...
function statusForDay(plan, row) {
  ...
  const real = pnlOnDate(plan.kind, row.date);
  const minGoal = actualBase * plan.minPct;
  const targetGoal = actualBase * plan.targetPct;
  ...
  return { actualBase, real, minGoal, targetGoal, status, tone, nextActual: Math.max(0, actualBase + real) };
}
...
function renderPlan(kind) {
  ...
  const monthClosed = closedPlanTrades(kind).filter(t => t.fechaCierre >= monthStartDate && t.fechaCierre <= monthEndDate);
  const pnlMes = monthClosed.reduce((s,t)=>s+n(t.pnl),0);
  const dayTrades = tradesOnDate(kind, curr.date);
  const progressVsTarget = currStatus.targetGoal > 0 ? Math.min((currStatus.real / currStatus.targetGoal) * 100, 150) : 0;
  const monthlyTargetPct = (Math.pow(1 + plan.targetPct, plan.diasMes) - 1);

  return `
    <div class="btn-row">
      <button class="btn ${plan.isAcc?'btn-green':'btn-blue'}" onclick="openPlanPosition('${kind}', ${day})">+ Registrar trade del día ${day}</button>
      <button class="btn btn-red" onclick="openModal('cerrar-pos')">✕ Cerrar posición</button>
      <button class="btn" onclick="openModal('config')">⚙ Configurar plan</button>
    </div>

    <div class="alert-info">
      Este plan es una guía de disciplina: arrancas con <strong>${money(plan.planCapital)}</strong>, buscas un rango de <strong>${(plan.minPct*100).toFixed(1)}% a ${(plan.targetPct*100).toFixed(1)}%</strong> por trade/día y EVA compara tu resultado real contra ese rango.
    </div>
    ...
    <div class="kpi-grid">
      ...
      ${kpi('Operación sugerida hoy', money(currStatus.actualBase), `Día ${day} de ${plan.diasMes}`, plan.isAcc?'teal':'blue')}
      ${kpi('Mínimo esperado', money(currStatus.minGoal), `+${(plan.minPct*100).toFixed(1)}%`, 'amber')}
      ${kpi('Meta ideal', money(currStatus.targetGoal), `+${(plan.targetPct*100).toFixed(1)}%`, plan.isAcc?'teal':'blue')}
      ${kpi('Siguiente según real', money(currStatus.nextActual), currStatus.real ? `Resultado: ${money(currStatus.real)}` : 'Sin cierre aún', currStatus.real>=0?'teal':'red')}
      ${kpi('Meta mensual compuesta', (monthlyTargetPct*100).toFixed(1)+'%', `${plan.diasMes} días operables`, 'amber')}
      ${kpi('Capital final proyectado', money(final12), mxn(final12), plan.isAcc?'teal':'blue')}
    </div>
    ...
    <div class="card">
      <div style="display:flex;justify-content:space-between;font-size:13px;margin-bottom:8px;gap:10px;flex-wrap:wrap">
        <span style="color:var(--muted)">Fecha: <strong>${dateText(curr.date)}</strong> · Base real: <strong>${money(currStatus.actualBase)}</strong></span>
        <span>Real: <strong class="${currStatus.real>=0?'green':'red'}">${money(currStatus.real)}</strong> / Meta ideal: <strong>${money(currStatus.targetGoal)}</strong></span>
      </div>
      ...
      <div class="prog-label"><span class="badge ${currStatus.tone}">${currStatus.status}</span> ${currStatus.real ? (currStatus.real >= currStatus.targetGoal ? 'Excelente, vas arriba del plan.' : currStatus.real >= currStatus.minGoal ? 'Bien, cumpliste el mínimo.' : 'Vas debajo de lo esperado para este día.') : 'Aún no hay trade cerrado para este día.'}</div>
    </div>

    <div class="sec-title">Trades del día seleccionado</div>
    <div class="card">
      ${dayTrades.length === 0 ? `<p style="color:var(--muted);margin-bottom:10px">No hay trades registrados para ${dateText(curr.date)}.</p>` :
      `<div class="tbl-wrap"><table>
        <thead><tr><th>Activo</th><th>Estado</th><th>Fecha apertura</th><th>Fecha cierre</th><th>Inversión</th><th>P&L</th><th>P&L %</th><th>Siguiente real</th><th></th></tr></thead>
        <tbody>${dayTrades.map(t => {
          const isClose = t.estado === 'Cerrada';
          const nextOp = isClose ? money(Math.max(0, n(t.subtotal) + n(t.pnl))) : '—';
          return `<tr>
            ...
            <td><button class="btn btn-sm" onclick="openEditTrade('${plan.isAcc?'acc':'opc'}','${esc(tradeSymbol(t, plan.isAcc))}')">✏</button></td>
          </tr>`;
        }).join('')}</tbody>
      </table></div>`}
      <button class="btn ${plan.isAcc?'btn-green':'btn-blue'}" style="margin-top:10px" onclick="openPlanPosition('${kind}', ${day})">+ Registrar trade para este día</button>
    </div>
    ...
    <div class="card">
      <div class="tbl-wrap"><table class="plan-day-table">
        <thead><tr><th>Día</th><th>Fecha</th><th>Base real sugerida</th><th>Mínimo</th><th>Meta ideal</th><th>Resultado real</th><th>Estado</th><th></th></tr></thead>
        <tbody>${daily.map(r => { const st = statusForDay(plan, r); return `<tr class="${r.day===day?'current-day':''}">
          ...
          <td><button class="btn btn-sm" onclick="openPlanPosition('${kind}',${r.day})">Registrar trade</button></td>
        </tr>`; }).join('')}</tbody>
      </table></div>
    </div>
    ...
    <div class="card">
      ${monthClosed.length ? `<div class="tbl-wrap"><table>
        <thead><tr><th>Activo</th><th>Cierre</th><th>Inversión</th><th>P&L</th><th>P&L %</th></tr></thead>
...
function drawPlanChart(kind) {
  ...
  chartInstances[kind] = new Chart(canvas, {
    ...
    data: {
      ...
      datasets: [
        { label: 'USD', data: months.map(m => +m.end.toFixed(2)), borderColor: plan.isAcc ? '#1D9E75' : '#185FA5', backgroundColor: plan.isAcc ? 'rgba(29,158,117,.1)' : 'rgba(24,95,165,.1)', borderWidth: 2.5, pointRadius: 4, fill: true, tension: 0.3 },
...
function openModal(type) {
  ...
  if (type === 'nueva-pos-accion') {
    ...
    const ctxInfo = ctx ? `<div class="alert-info">Trade del plan · Día ${ctx.day} · Fecha ${dateText(ctx.date)} · Capital sugerido: <strong>${money(ctx.amount)}</strong> · Rango: ${(ctx.minPct*100).toFixed(1)}%-${(ctx.targetPct*100).toFixed(1)}%</div>` : '';
