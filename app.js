// ============================================================
// EVA — app.js v22
// Fixes: lectura clara de disponible real, capital ocupado, flotante y capital potencial al objetivo.
//        El compound sigue usando capital cerrado; el dashboard muestra realidad operativa.
// ============================================================
...
function renderDashboard() {
  ...
  return `
    ...
    <div class="alert-info"><strong>Lectura de capital:</strong> Disponible real = dinero libre/buying power capturado en tu cuenta. Capital ocupado = dinero dentro de trades abiertos. Flotante = ganancia/pérdida temporal de posiciones abiertas. Capital potencial al objetivo = disponible + ocupado + ganancia esperada si cierras en meta.</div>
    ...
    <div class="kpi-grid">
      ...
      ${kpi('Capital histórico', money(d.capitalConsolidado), 'Base configurada + P&L cerrado', 'blue')}
...
function renderPlan(kind) {
  ...
  const disponibleCuenta = cuentaDisponibleReal();
  const disponibleReal = disponibleCuenta == null ? Math.max(0, capitalConsolidado - usedTotal) : Math.max(0, disponibleCuenta);
  const metaPendientePlan = openTrades.reduce((s,t)=>s + (plan.isAcc ? openTargetProfitAcc(t) : openTargetProfitOpc(t)), 0);
  const capitalTotalEstrategico = disponibleReal + usedTotal;
  const valorTotalEstimado = disponibleReal + usedTotal + floatingTotal;
  const capitalPotencialObjetivo = disponibleReal + usedTotal + metaPendientePlan;
  ...
  return `
    ...
    <div class="kpi-grid compact-plan">
      ${kpi('Disponible real', money(disponibleReal), `${mxn(disponibleReal)} · libre para operar hoy`, disponibleReal>=0?'teal':'red')}
      ${kpi('Capital ocupado', money(usedTotal), `${openTrades.length} posición(es) abierta(s)`, usedTotal>0?'amber':'teal')}
      ${kpi('Capital total estratégico', money(capitalTotalEstrategico), 'Disponible + ocupado en este plan', 'blue')}
      ...
      ${kpi('Valor estimado actual', money(valorTotalEstimado), 'Disponible + posiciones valuadas', valorTotalEstimado>=0?'teal':'red')}
      ${kpi('Capital potencial al objetivo', money(capitalPotencialObjetivo), `Meta pendiente: ${money(metaPendientePlan)}`, capitalPotencialObjetivo>=0?'teal':'red')}
      ${kpi('Capital del plan', money(capitalConsolidado), 'Base + P&L cerrado · mueve compound', 'blue')}
