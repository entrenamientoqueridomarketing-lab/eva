...
//        opciones por operación con compuesto visible, conciliación y reset seguro a cero
// ============================================================
  ...
  return `
    ...
    <div id="csv-preview-wrap" class="hidden">
      ...
      <div class="btn-row">
        ...
        <button class="btn" onclick="importOpenPositionsOnly(false)">Solo importar posiciones abiertas</button>
...
async function importOpenPositionsOnly(resetFirst = false) {
  ...
  if (!openStocks.length && !openOptions.length) {
    showToast('No hay posiciones abiertas para importar.', 'info');
    return;
  }

  try {
    showToast('Importando posiciones abiertas detectadas...', 'info');
    ...
    await syncAll();
    showToast(`Posiciones abiertas importadas: ${ok}.`, 'success');
  } catch(e) {
...
async function acceptFullReconciliation() {
  if (!bankParsed) { showToast('Primero analiza la conciliación.', 'error'); return; }
  const ok = window.confirm('Aceptar conciliación completa va a restaurar operaciones anteriores, guardar el capital real estimado y cargar solo las posiciones abiertas detectadas. Esto evita doble conteo del P&L histórico. ¿Continuamos?');
  if (!ok) return;
  try {
    ...
    renderAll();
    showToast('Conciliación completa aplicada sin doble conteo.', 'success');
  } catch(e) {
