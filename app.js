// ============================================================
// EVA — app.js v17
// Fixes: planes simplificados, unidades operativas reales, incluir/excluir del plan,
//        P&L realizado + flotante en el avance y orden natural de trades
// ============================================================
...
function isIncludedInPlan(t) {
  ...
  return !(v === 'fuera_plan' || v === 'excluir' || v === 'excluido' || v === 'no_plan' || v === 'no plan');
}
function planIncludeText(t) { return isIncludedInPlan(t) ? 'Incluido' : 'Fuera del plan'; }
...
function unitTradesWithFallback(plan) {
  ...
  if (plan.isAcc) {
    ...
    let idx = 0;
    return all.map(t => {
      const d = tradeOpenDate(t, true) || t.fechaCierre || `sin-fecha-${n(t.rowIndex)}`;
...
function monthSummaryForPlan(plan, month) {
  ...
  const closed = trades.filter(t => t.estado === 'Cerrada');
  const pnl = closed.reduce((s,t)=>s+n(t.pnl),0);
  const flotante = plan.isAcc ? trades.filter(t => t.estado === 'Abierta').reduce((s,t)=>s+unrealizedAcc(t),0) : 0;
  const neto = pnl + flotante;
  const balanceInicial = actualCapitalBeforeDay(plan, rowForGlobalUnit(plan, unitStart));
  const balanceActual = balanceInicial + neto;
  ...
  return { month, balanceInicial, balanceActual, pnl, flotante, neto, metaEstimada, diff, estado, tone, trades, closed };
}
  ...
  return `
    ...
    <div class="month-summary-grid">
      ${months.map(m => { const ms = monthSummaryForPlan(plan, m.month); return `<button class="month-summary-card ${m.month===month?'active':''}" onclick="selectPlanMonth('${kind}',${m.month})">
        ...
        <span class="${ms.neto>=0?'green':'red'}">Ganando/perdiendo: ${money(ms.neto)}</span>
...
async function doEditAccion(rowIndex) {
  const payload = {
    ...
    planTipo: (($('e-incluir-plan')?.value || 'si') === 'si') ? '' : 'fuera_plan',
