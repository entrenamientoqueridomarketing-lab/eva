...
function actualCapitalBeforeDay(plan, row) {
  ...
  // Capital real acumulado antes de la unidad: realizado + flotante previo.
  return Math.max(0, plan.planCapital + planNetPnl(plan, row.globalDay - 1));
}
...
function planCurrentCapital(plan, maxUnit = Infinity) {
  return Math.max(0, plan.planCapital + planNetPnl(plan, maxUnit));
}
...
function renderPlan(kind) {
  ...
  const accountTotal = planCurrentCapital(plan);
  const strategyCapital = accountTotal;
  ...
  return `
    ...
    <div class="kpi-grid compact-plan">
      ${kpi('Capital real del plan', money(accountTotal), mxn(accountTotal), 'teal')}
      ${kpi('Capital '+plan.label.toLowerCase(), money(strategyCapital), `Incluye realizado${plan.isAcc?' + flotante':''}: ${money(realizedTotal + floatingTotal)}`, plan.tone)}
    ...
    <div class="month-summary-grid">
      ${months.map(m => { const ms = monthSummaryForPlan(plan, m.month); return `<button class="month-summary-card ${m.month===month?'active':''}" onclick="selectPlanMonth('${kind}',${m.month})">
        ...
        <span class="${n(ms.neto)>=0?'green':'red'}">Ganando/perdiendo: ${renderPlanMoneyOrDash(ms.neto)}</span>
