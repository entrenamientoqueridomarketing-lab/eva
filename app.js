// ============================================================
// EVA — app.js v14
// Fixes: Conciliación Bancaria con flujo seguro de aceptación, selección de posiciones abiertas,
//        historial solo para cálculo, opciones por operación y planes vivos
// ============================================================
...
function getDash() {
  ...
  const unrealAcc = counted(D.acciones).filter(a => a.estado === 'Abierta').reduce((s,a) => s + unrealizedAcc(a), 0);
  // Las primas de opciones no se pueden traer de forma confiable con GOOGLEFINANCE; quedan fuera hasta capturar precio actual manual.
  const unrealOpc = 0;
  const unreal = unrealAcc + unrealOpc;
  const bookTotal = base + pnl;
  const marketTotal = bookTotal + unreal;
  const comm = [...counted(D.acciones), ...counted(D.opciones)].reduce((s,x) => s + n(x.comision), 0);
  return { capAcc: c.capAcc, capOpc: c.capOpc, capTotal: marketTotal, capTotalBook: bookTotal, pnlAcc, pnlOpc, pnl, unrealAcc, unrealOpc, unreal, pnlTotalMercado: pnl + unreal, comm, rend: base ? (pnl + unreal) / base : 0 };
}
...
function renderDashboard() {
  ...
  return `
    ...
    <div class="alert-info">El capital total estimado incluye P&L realizado y P&L no realizado de acciones cuando Google Sheets ya actualizó GOOGLEFINANCE. Las opciones no tienen cotización automática confiable; se valoran al cerrar o al capturar manualmente.</div>
    ...
    <div class="kpi-grid">
      ...
      ${kpi('Capital total estimado', money(d.capTotal), `${mxn(d.capTotal)} · contable ${money(d.capTotalBook)}`, n(d.capTotal)>=0?'teal':'red')}
      ${kpi('P&L realizado', money(d.pnl), mxn(d.pnl), n(d.pnl) >= 0 ? 'teal' : 'red')}
      ${kpi('P&L no realizado', money(d.unreal), `${mxn(d.unreal)} · acciones con GOOGLEFINANCE`, n(d.unreal) >= 0 ? 'teal' : 'red')}
    ...
    <div class="card">
      ...
      <div class="tbl-wrap"><table>
        <thead><tr><th>Activo</th><th>Tipo</th><th>Entrada</th><th>Actual</th><th>Inversión</th><th>Valor mercado</th><th>P&L no realizado</th><th>Meta salida</th><th>Meta P&L</th><th>Fecha</th><th></th></tr></thead>
        <tbody>
...
function planGlobalUnitForTrade(t, plan, fallbackIndex = null) {
  ...
  if (plan.isAcc) {
    const d = tradeOpenDate(t, true) || t.fechaCierre || '';
...
function unitTradesWithFallback(plan) {
  // v16: el plan avanza por unidades operativas, no por calendario fijo.
  // Acciones: todos los trades del mismo día pertenecen a la misma unidad.
  // Opciones: cada operación/contrato registrado avanza como unidad operativa, salvo que ya venga ligado explícitamente.
  const all = sortTradesForPlan(getPlanTrades(plan.kind), plan.isAcc);
  if (plan.isAcc) {
    const dateToUnit = {};
    let idx = 0;
    return all.map(t => {
      let unit = n(t.planGlobalUnit);
      if (!unit) {
        const d = tradeOpenDate(t, true) || t.fechaCierre || 'sin-fecha';
        if (!dateToUnit[d]) dateToUnit[d] = ++idx;
        unit = dateToUnit[d];
      }
      return { ...t, _planUnitResolved: unit };
    });
  }
  let optionIndex = 0;
  return all.map(t => {
    let unit = n(t.planGlobalUnit);
    if (!unit) unit = ++optionIndex;
    return { ...t, _planUnitResolved: unit };
  });
}
...
function actualCapitalBeforeDay(plan, row) {
  ...
  return Math.max(0, plan.planCapital + pnlBeforePlanUnit(plan, row.globalDay));
}
...
function statusForDay(plan, row) {
  ...
  let tone = 'badge-gray';
  if (usage.openUsed > 0 && usage.closedUsed === 0) { status = `${usage.countedTrades.length} trade(s) abierto(s)`; tone = 'badge-amber'; }
  if (usage.closedUsed > 0 || real !== 0) {
    if (real >= targetGoal) { status = usage.openUsed > 0 ? 'Meta cumplida + abierto' : 'Arriba de meta'; tone = 'badge-green'; }
    else if (real >= minGoal) { status = usage.openUsed > 0 ? 'Mínimo cumplido + abierto' : 'Cumplió mínimo'; tone = 'badge-blue'; }
    else if (real > 0) { status = usage.openUsed > 0 ? 'Debajo + abierto' : 'Abajo de lo esperado'; tone = 'badge-amber'; }
    else { status = usage.openUsed > 0 ? 'Pérdida + abierto' : 'Pérdida'; tone = 'badge-red'; }
  }
...
function weekStatsForPlan(plan, selectedGlobalUnit) {
  ...
  const pnl = closed.reduce((s,t)=>s+n(t.pnl),0);
  const gains = closed.filter(t=>n(t.pnl)>0).reduce((s,t)=>s+n(t.pnl),0);
  const losses = closed.filter(t=>n(t.pnl)<0).reduce((s,t)=>s+n(t.pnl),0);
  ...
  return { weekStart, weekEnd, trades, closed, pnl, gains, losses, commissions, base, idealGoal, minGoal, avance, faltante, avgReturn, phrase, tone };
}
function monthSummaryForPlan(plan, month) {
  ...
  const trades = unitTradesWithFallback(plan).filter(t => isCounted(t) && n(t._planUnitResolved) >= unitStart && n(t._planUnitResolved) <= unitEnd);
  const closed = trades.filter(t => t.estado === 'Cerrada');
  const pnl = closed.reduce((s,t)=>s+n(t.pnl),0);
  ...
  const diff = balanceActual - metaEstimada;
  let estado = 'Sin datos';
  let tone = 'gray';
  if (closed.length) {
    if (diff >= 0) { estado = 'Arriba del plan'; tone = 'green'; }
    else if (balanceActual >= (m?.endMin || balanceInicial)) { estado = 'Cerca del plan'; tone = 'amber'; }
    else { estado = 'Debajo del plan'; tone = 'red'; }
  }
  return { month, balanceInicial, balanceActual, pnl, metaEstimada, diff, estado, tone, trades, closed };
}
function renderPlan(kind) {
  ...
  const allTrades = unitTradesWithFallback(plan).filter(isCounted);
  ...
  const accountTotal = n(D.dashboard?.capTotal) || (n(cfg().capAcc)+n(cfg().capOpc));
  const strategyCapital = plan.capCuenta || plan.planCapital;
  const freeCapital = Math.max(0, strategyCapital - usedTotal);
  ...
  return `
    ...
    <div class="alert-info">
      <strong>${plan.label}:</strong> combina tu operación real con interés compuesto. El plan avanza por <strong>${unitLabel}s</strong>, no por calendario fijo. Todo trade contabilizado de ${plan.label.toLowerCase()} suma al plan.
    </div>

    <div class="kpi-grid compact-plan">
      ${kpi('Capital real cuenta', money(accountTotal), mxn(accountTotal), 'teal')}
      ${kpi('Capital '+plan.label.toLowerCase(), money(strategyCapital), `${(plan.uso*100).toFixed(0)}% operable: ${money(strategyCapital*plan.uso)}`, plan.tone)}
      ${kpi('Capital usado', money(usedTotal), `${openTrades.length} posición(es) abierta(s)`, usedTotal>0?'amber':'teal')}
      ${kpi('Capital restante', money(freeCapital), 'Disponible para nuevas entradas', freeCapital>=0?'teal':'red')}
      ...
      ${kpi('% avance semanal', Math.max(0, week.avance).toFixed(1)+'%', week.phrase, week.tone)}
      ${kpi('% faltante semanal', week.idealGoal ? money(week.faltante) : '—', 'Para meta ideal semanal', week.faltante<=0?'teal':'amber')}
    </div>
    ...
    <div class="card">
      ...
      <div class="prog-label">Avance ${Math.max(0, week.avance).toFixed(1)}% · Faltante: <strong>${money(week.faltante)}</strong></div>
    </div>
    ...
    <div class="card">
      ...
      `<div class="tbl-wrap"><table>
        <thead><tr><th>Activo</th><th>Estado</th><th>Capital usado</th><th>Entrada</th><th>Cierre</th><th>Resultado</th><th></th></tr></thead>
    ...
    <div class="month-summary-grid">
      ${months.map(m => { const ms = monthSummaryForPlan(plan, m.month); return `<button class="month-summary-card ${m.month===month?'active':''}" onclick="selectPlanMonth('${kind}',${m.month})">
        ...
        <span>Balance actual: ${money(ms.balanceActual)}</span>
        <span class="${ms.pnl>=0?'green':'red'}">Ganando/perdiendo: ${money(ms.pnl)}</span>
        <span>Meta estimada: ${money(ms.metaEstimada)}</span>
        <small class="${ms.diff>=0?'green':'red'}">${ms.estado} · ${ms.diff>=0?'+':''}${money(ms.diff)}</small>
      </button>`; }).join('')}
    </div>
    ...
    <div class="card">
      <div class="tbl-wrap"><table class="plan-day-table simplified">
        <thead><tr><th>Unidad</th><th>Capital inicial</th><th>Usado</th><th>Libre</th><th>Meta mín.</th><th>Meta ideal</th><th>Resultado</th><th>Nuevo capital</th><th>Estado</th><th></th></tr></thead>
...
function drawPlanChart(kind) {
  ...
  for (let u=1; u<=totalUnits; u++) {
    ...
    real.push(+(actualCapitalBeforeDay(plan, rr)).toFixed(2));
  ...
  return `
    ...
    <div class="card">
      ...
      <div class="alert-warn" style="margin-top:10px">
        Usa <strong>Restaurar todo a cero</strong> solo antes de iniciar una conciliación completa. Borra operaciones, socios y watchlist, y pone capitales en $0. Conserva tu nombre, porcentajes/metas y tipo de cambio.
      </div>
    </div>
...
async function doNuevaAccion() {
  ...
  try {
    const ctx = currentPlanContext && currentPlanContext.kind === 'acciones' ? currentPlanContext : null;
    await api('saveAccion', { ticker, acciones, precioCompra: precio, comision, fechaCompra: fecha, contabiliza: ($('m-contabiliza')?.value || 'si') === 'si', planTipo: ctx ? 'acciones' : '', planMes: ctx ? ctx.month : '', planUnidad: ctx ? ctx.day : '', planGlobalUnit: ctx ? ctx.globalDay : '', planFecha: ctx ? ctx.date : '', accountName: $('m-cuenta')?.value || accountNameDefault() });
