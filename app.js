...
function cuentaOptions(selected = '') {
  ...
  if (!cuentas.length) return '<option value="">Cuenta general / sin asignar</option>';
  return '<option value="">Cuenta general / sin asignar</option>' + cuentas.map(c => {
...
function renderImportador() {
  return `
    ...
    <div class="card">
      ...
      <div class="form-grid"><div class="form-group"><label>¿A qué cuenta pertenece esta conciliación?</label><select id="recon-account"><option value="">Cuenta general / sin asignar</option>${(D.cuentas||[]).map(c=>`<option value="${esc(c.nombre)}">${esc(c.nombre)}${c.cuentaPadre ? ' · ' + esc(c.cuentaPadre) : ''}</option>`).join('')}</select></div></div>
...
function reconciliationSuggestedCapital() {
  ...
  return {
    ...
    accountName: $('recon-account')?.value || ''
  };
}
...
async function doNuevaAccion() {
  ...
  try {
    const ctx = currentPlanContext && currentPlanContext.kind === 'acciones' ? currentPlanContext : null;
    await api('saveAccion', { ticker, acciones, precioCompra: precio, comision, fechaCompra: fecha, contabiliza: ($('m-contabiliza')?.value || 'si') === 'si', planTipo: ctx ? 'acciones' : '', planMes: ctx ? ctx.month : '', planUnidad: ctx ? ctx.day : '', planGlobalUnit: ctx ? ctx.globalDay : '', planFecha: ctx ? ctx.date : '', accountName: $('m-cuenta')?.value || '' });
