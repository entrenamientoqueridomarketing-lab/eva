const EVA_BUILD_VERSION = '27.1';
// ============================================================
// EVA — app.js v26
// Fixes: el capital libre visible del plan ahora refleja lo disponible de la unidad actual.
//        Corrige Ruta Financiera: persistencia de % y evita doble conteo de capital semilla.
//        Evita que el usuario vea $950 como libre si ya abrió una operación con ese capital.
// ============================================================

const EVA_VERSION = 'v27.0';
...
let chartInstances = {};
let selectedPlanChart = { acciones: 'capital', opciones: 'capital' };
let lastTradeImageData = { acciones: null, opciones: null };
...
function dateInputValue(v, fallback = todayISO) {
  const iso = cleanISODate(v);
...
function hasLegacyPlanMarker(t) {
  // Compatibilidad con capturas/versiones anteriores: algunas filas tienen "SI" en contabiliza,
  // o columnas de unidad/fecha, pero no alcanzaron a guardar planTipo.
  return n(t?.planMes) > 0 || n(t?.planUnidad) > 0 || n(t?.planGlobalUnit) > 0 || !!t?.planFecha;
}
function isLegacyIncludedInPlan(t) {
  if (!isCounted(t)) return false;
  const v = String(t?.planTipo || '').trim().toLowerCase();
  if (v) return false;
  // 1) Si tiene unidad heredada, sí forma parte del plan.
  if (hasLegacyPlanMarker(t)) return true;
  // 2) Si ya está cerrado y contabilizado, lo leemos como resultado histórico del plan.
  //    Esto evita que al actualizar a v25 se vea todo en cero.
  // 3) Si está abierto y sin unidad, NO se integra todavía: queda como capital pendiente de liberar.
  return String(t?.estado || '').toLowerCase() === 'cerrada' && !!t?.fechaCierre;
}
function isIncludedInPlan(t) {
  if (!isCounted(t)) return false;
  const v = String(t?.planTipo || '').trim().toLowerCase();
  if (!v) return isLegacyIncludedInPlan(t);
  if (v === 'fuera_plan' || v === 'excluir' || v === 'excluido' || v === 'no_plan' || v === 'no plan') return false;
...
function planIncludeText(t) {
  ...
  if (isLegacyIncludedInPlan(t)) return 'Incluido (compat.)';
...
function dateText(v) {
  ...
  return d ? d.toLocaleDateString('es-MX', { day: '2-digit', month: 'short', year: 'numeric' }) : esc(v);
}
...
function freedomLocalOverrides() {
  try {
    const raw = localStorage.getItem('eva_libertad_config');
    return raw ? JSON.parse(raw) : {};
  } catch(e) { return {}; }
}
function freedomCfg() {
  // v26.4: las configuraciones de libertad financiera tienen respaldo local
  // para evitar que el % mensual o meses de proyección regresen a defaults
  // si Apps Script tarda en devolver la Config actualizada.
  const c = { ...(D.config || {}), ...freedomLocalOverrides() };
  return {
    gastoTranquiloMXN: safeMoney(c.libertadGastoTranquiloMXN, 60000) || 60000,
    gastoIdealMXN: safeMoney(c.libertadGastoIdealMXN, 150000) || 150000,
    rendimientoMensual: safePct(c.libertadRendimientoMensual, 0.10) || 0.10,
    retiroMensualUSD: safeMoney(c.libertadRetiroMensualUSD, 1000) || 1000,
    capitalProtegidoUSD: safeMoney(c.libertadCapitalProtegidoUSD, 10000) || 10000,
    metaEtapaUSD: safeMoney(c.libertadMetaEtapaUSD, 10000) || 10000,
    metaAmpliaUSD: safeMoney(c.libertadMetaAmpliaUSD, 50000) || 50000,
    mesesProyeccion: Math.max(3, Math.min(120, Math.round(n(c.libertadMesesProyeccion || 24))))
  };
}
function monthsToReachTarget(startCapital, targetCapital, monthlyReturn, monthlyWithdrawal, protectedCapital) {
  let capital = Math.max(0, n(startCapital));
  const target = Math.max(0, n(targetCapital));
  const r = Math.max(0, n(monthlyReturn));
  const w = Math.max(0, n(monthlyWithdrawal));
  const protectedBase = Math.max(0, n(protectedCapital));
  if (capital >= target) return 0;
  for (let m = 1; m <= 600; m++) {
    const gain = capital * r;
    const canWithdraw = capital >= protectedBase && (capital + gain - w) >= protectedBase;
    capital = capital + gain - (canWithdraw ? w : 0);
    if (capital >= target) return m;
    if (r <= 0 && !canWithdraw) return Infinity;
  }
  return Infinity;
}
function freedomMotivation(seed = 0) {
  const a = [
    'Vas construyendo la máquina, no la estás consumiendo', 'El plan premia al disciplinado', 'Una buena salida protege diez buenas entradas', 'Tu capital base es sagrado', 'Hoy se avanza con control', 'La paciencia también paga', 'El objetivo no es operar más, es operar mejor', 'Sigue el sistema, no el impulso', 'El compound necesita oxígeno', 'La libertad se construye por etapas'
  ];
  const b = [
    'mantén el tamaño correcto', 'respeta el capital protegido', 'no confundas oportunidad con obligación', 'deja que el ciclo trabaje', 'protege lo ganado', 'mide antes de retirar', 'separa capital del plan y capital atrapado', 'cierra con intención', 'la consistencia pesa más que el golpe perfecto', 'si el mercado no da entrada, no le inventes una'
  ];
  const c = [
    '🔥', '💚', '📈', '🧠', '🛡️', '🎯', '⚙️', '🚀', '💪', '✅'
  ];
  const i = Math.abs(Math.round(seed)) % 1000;
  return `${a[i % 10]}; ${b[Math.floor(i / 10) % 10]}. ${c[Math.floor(i / 100) % 10]}`;
}
function currentStageForCapital(capital, configuredTarget) {
  const milestones = [10000, 25000, 50000, 100000, 250000, 500000, 1000000];
  const custom = n(configuredTarget);
  if (custom && !milestones.includes(custom)) milestones.push(custom);
  milestones.sort((a,b)=>a-b);
  const target = milestones.find(m => capital < m) || milestones[milestones.length - 1];
  const idx = milestones.indexOf(target);
  const prev = idx > 0 ? milestones[idx - 1] : 0;
  return { target, prev, milestones };
}
function currentPlanFreeForNewEntries(plan) {
  // Capital realmente libre para nuevas entradas dentro de cada motor.
  // No debe tomar el buying power global manual ni capital de configuración viejo.
  try {
    const unit = currentPlanUnit(plan);
    const row = rowForGlobalUnit(plan, unit);
    const st = statusForDay(plan, row);
    let free = Math.max(0, n(st.remaining));
    // v26.4: NO usamos capCuenta/manual para calcular libre del plan.
    // Ese campo puede venir inflado por configuración vieja (ej. $1,100).
    // El libre real del motor sale de la unidad actual del plan.
    return free;
  } catch(e) {
    return Math.max(0, planReinvestableCapital(plan));
  }
}
function freedomMotorState(plan) {
  const consolidado = planConsolidatedCapital(plan);
  const pendiente = planPendingReleaseCapital(plan);
  const ocupado = planOccupiedCapital(plan);
  const libre = currentPlanFreeForNewEntries(plan);
  // Capital operativo para la meta: lo que ya pertenece al motor + lo pendiente de liberar.
  // Capital libre para operar: solo lo disponible para nuevas entradas.
  const operativo = Math.max(0, consolidado + pendiente);
  return { consolidado, pendiente, ocupado, libre, operativo };
}
function getFreedomState() {
  const fc = freedomCfg();
  const planAcc = planSettings('acciones');
  const planOpc = planSettings('opciones');
  const accState = freedomMotorState(planAcc);
  const opcState = freedomMotorState(planOpc);
  const accConsolidado = accState.consolidado;
  const opcConsolidado = opcState.consolidado;
  const accPendiente = accState.pendiente;
  const opcPendiente = opcState.pendiente;
  const accLibre = accState.libre;
  const opcLibre = opcState.libre;
  const accOperativo = accState.operativo;
  const opcOperativo = opcState.operativo;
  const capitalConsolidado = accConsolidado + opcConsolidado;
  const capitalPendiente = accPendiente + opcPendiente;
  const capitalLibrePlanes = accLibre + opcLibre;
  const capitalOperativo = accOperativo + opcOperativo;
  const stage = currentStageForCapital(capitalOperativo, fc.metaEtapaUSD);
  const stageSpan = Math.max(1, stage.target - stage.prev);
  const stageProgress = Math.max(0, Math.min(1, (capitalOperativo - stage.prev) / stageSpan));
  const ingresoMensualUSD = capitalOperativo * fc.rendimientoMensual;
  const retiro = fc.retiroMensualUSD;
  const puedeRetirar = capitalOperativo >= fc.capitalProtegidoUSD;
  const capitalDespuesRetiro = capitalOperativo + ingresoMensualUSD - retiro;
  const tocaBase = capitalDespuesRetiro < fc.capitalProtegidoUSD;
  const rendimientoNecesarioRetiro = capitalOperativo ? retiro / capitalOperativo : 0;
  const gastoTranquiloUSD = fc.gastoTranquiloMXN / FX;
  const gastoIdealUSD = fc.gastoIdealMXN / FX;
  return {
    ...fc,
    planAcc, planOpc,
    accConsolidado, opcConsolidado, accPendiente, opcPendiente, accLibre, opcLibre, accOperativo, opcOperativo,
    capitalConsolidado, capitalPendiente, capitalLibrePlanes, capitalOperativo,
    stage, stageProgress,
    faltanteEtapa: Math.max(0, stage.target - capitalOperativo),
    ingresoMensualUSD, ingresoMensualMXN: ingresoMensualUSD * FX,
    puedeRetirar, capitalDespuesRetiro, tocaBase, rendimientoNecesarioRetiro,
    gastoTranquiloUSD, gastoIdealUSD,
    coberturaTranquilo: gastoTranquiloUSD ? ingresoMensualUSD / gastoTranquiloUSD : 0,
    coberturaIdeal: gastoIdealUSD ? ingresoMensualUSD / gastoIdealUSD : 0,
    mesesAEtapa: monthsToReachTarget(capitalOperativo, stage.target, fc.rendimientoMensual, 0, fc.capitalProtegidoUSD),
    mesesA50k: monthsToReachTarget(capitalOperativo, 50000, fc.rendimientoMensual, 0, fc.capitalProtegidoUSD),
    mesesA100k: monthsToReachTarget(capitalOperativo, 100000, fc.rendimientoMensual, 0, fc.capitalProtegidoUSD)
  };
}
function monthsText(m) {
  if (!isFinite(m)) return 'No estimable';
  if (m === 0) return 'Meta alcanzada';
  return `${m} mes(es)`;
}
...
function showPage(page) {
  ...
  const titles = { dashboard:'Dashboard', capital:'Capital', posiciones:'Posiciones', trades:'Trades', 'plan-acciones':'Plan Acciones', 'plan-opciones':'Plan Opciones', socios:'Inversionistas', cuentas:'Cuentas/Subcuentas', watchlist:'Watchlist', importador:'Conciliación Bancaria', reportes:'Reportes', libertad:'Ruta de Libertad Financiera' };
...
function renderPage() {
  const renders = {
    ...
    importador: renderImportador, reportes: renderReportes, libertad: renderLibertad
  };
  try {
    $('page-content').innerHTML = (renders[currentPage] || renderDashboard)();
  } catch (err) {
    console.error('Error renderizando página', currentPage, err);
    $('page-content').innerHTML = `<div class="alert-warn"><strong>EVA no pudo abrir esta sección.</strong><br>${esc(err && err.message ? err.message : String(err))}</div>`;
    return;
  }
  // Post-render charts: si Chart.js falla o no carga, la página queda visible y solo se omiten las gráficas.
  if (currentPage === 'plan-acciones') setTimeout(() => { try { drawPlanChart('acciones'); } catch(e){ console.error(e); } }, 50);
  if (currentPage === 'plan-opciones') setTimeout(() => { try { drawPlanChart('opciones'); } catch(e){ console.error(e); } }, 50);
  if (currentPage === 'reportes') setTimeout(() => { try { drawReportChart(); } catch(e){ console.error(e); } }, 50);
  if (currentPage === 'libertad') setTimeout(() => { try { drawLibertadCharts(); } catch(e){ console.error(e); } }, 80);
}
...
function getDash() {
  ...
  const disponibleCuentaManual = cuentaDisponibleReal();
  // v26.3: el capital libre para operar ahora debe salir de los planes reales,
  // no del buying power manual capturado en Cuentas. El buying power queda como corte/referencia.
  const planAcc = planSettings('acciones');
  const planOpc = planSettings('opciones');
  const libreAccPlan = planReinvestableCapital(planAcc);
  const libreOpcPlan = planReinvestableCapital(planOpc);
  const disponibleReal = Math.max(0, libreAccPlan + libreOpcPlan);
  const metaPendiente = openTargetProfitAll(openAcc, openOpc);
  const capitalTotalEstrategico = disponibleReal + ocupado; // Capital libre para nuevas entradas + capital dentro de posiciones abiertas.
  const valorTotalEstimado = disponibleReal + ocupado + flotante; // Marcado a mercado: libre plan + valor actual estimado de posiciones.
  ...
  return {
    ...
    capitalReinvertible: disponibleReal, disponibleReal, disponibleCuentaManual, libreAccPlan, libreOpcPlan,
...
function getPeriod(period) {
  ...
  const ok = dv => { const d = parseISODateLocal(dv); return !!d && d >= from; };
...
function renderDashboard() {
  ...
  const closed10 = [...D.acciones.map(a=>({...a,kind:'acc',name:a.ticker})), ...D.opciones.map(o=>({...o,kind:'opc',name:o.empresa}))]
    ...
    .sort((a,b) => (parseISODateLocal(b.fechaCierre)?.getTime() || 0) - (parseISODateLocal(a.fechaCierre)?.getTime() || 0))
    .slice(0, 10);
  const alerts = D.opciones.filter(o => o.estado==='Abierta' && o.fechaVencim)
    .map(o => ({ ...o, days: daysUntilISO(o.fechaVencim) }))
  ...
  return `
    ...
    <div class="alert-info"><strong>Lectura de capital:</strong> Capital libre para operar ahora = dinero disponible dentro de tus planes de Acciones + Opciones. El buying power manual queda solo como corte/referencia de Charles. Saldo conciliado queda como histórico/referencia. En cada plan verás capital asignado, capital usado, capital libre y capital pendiente de liberar. Flotante = ganancia/pérdida temporal de posiciones abiertas.</div>
    ...
    <div class="btn-row">
      ...
      <button class="btn btn-blue"  onclick="openModal('actualizar-disponible-real')">💵 Actualizar disponible real</button>
      <button class="btn"           onclick="openModal('config')">⚙ Configuración</button>
    </div>

    <div class="kpi-grid">
      ${kpi('Capital libre para operar ahora', money(d.disponibleReal), `Acciones ${money(d.libreAccPlan||0)} + Opciones ${money(d.libreOpcPlan||0)} · buying power manual: ${d.disponibleCuentaManual==null?'no capturado':money(d.disponibleCuentaManual)}`, d.disponibleReal>=0?'teal':'red')}
      ...
      ${kpi('Referencia histórica', money(d.capitalConsolidado), 'Base configurada + P&L cerrado; no es disponible real', 'blue')}
      ${kpi('Rendimiento estimado', pct(d.rend), '', n(d.rend) >= 0 ? 'amber' : 'red')}
      ${kpi('Comisiones pagadas', '-' + money(d.comm), '', 'red')}
    </div>

    <div class="sec-title">Resumen libertad financiera</div>
    ${libertadSummaryCard(true)}
  ...
  return `
    ...
    <div class="card">
      ${openOpc.length ? `<div class="tbl-wrap"><table>
        ...
        <tbody>${openOpc.map(o => {
          const days = o.fechaVencim ? daysUntilISO(o.fechaVencim) : null;
...
function addPlanDays(startISO, offset) {
  const d = parseISODateLocal(startISO) || new Date();
...
function buildMonthlyFromPlan(plan) {
  let curMin = effectivePlanInitialCapital(plan);
  let curIdeal = effectivePlanInitialCapital(plan);
  const rows = [];
...
function addPlanUnitDate(plan, offset) {
  ...
  const start = parseISODateLocal(plan.startDate) || new Date();
  if (isNaN(start.getTime())) return todayISO;
...
function buildDailyFromPlan(plan, month) {
  // v26.3: usar el capital efectivo del plan, no un valor manual viejo inflado.
  let minProjected = effectivePlanInitialCapital(plan);
  let idealProjected = effectivePlanInitialCapital(plan);
...
function planGlobalUnitForTrade(t, plan, fallbackIndex = null) {
  ...
  if (plan.isAcc) {
    ...
    const a = parseISODateLocal(plan.startDate);
    const b = parseISODateLocal(d);
    if (!a || !b) return 0;
...
function actualCapitalBeforeDay(plan, row) {
  const base = effectivePlanInitialCapital(plan);
  if (row.globalDay <= 1) return base;
  // Capital consolidado antes de la unidad: solo P&L cerrado. El flotante se muestra, pero no mueve el compound.
  return Math.max(0, base + planRealizedPnl(plan, row.globalDay - 1));
}
...
function clampPct(v, max = 150) {
  const x = Number(v);
  if (!isFinite(x)) return 0;
  return Math.max(0, Math.min(x, max));
}
function selectedTab(kind) { return selectedPlanTab[kind] || 'resumen'; }
function setPlanTab(kind, tab) { selectedPlanTab[kind] = tab; renderPage(); }
function tabBtn(kind, key, label) {
  const active = selectedTab(kind) === key ? 'active' : '';
  return `<button class="tab-btn ${active}" onclick="setPlanTab('${kind}','${key}')">${label}</button>`;
}
function setPlanChart(kind, mode) { selectedPlanChart[kind] = mode; drawPlanChart(kind); }
function chartBtn(kind, mode, label) {
  const active = (selectedPlanChart[kind] || 'capital') === mode ? 'active' : '';
  return `<button class="chip-btn ${active}" onclick="setPlanChart('${kind}','${mode}')">${label}</button>`;
}
function renderPlanTradesTable(plan, trades, emptyText = 'Sin operaciones para mostrar.') {
  if (!trades || !trades.length) return `<p style="color:var(--muted);margin-bottom:0">${emptyText}</p>`;
  return `<div class="tbl-wrap"><table>
    <thead><tr><th>Activo</th><th>Estado</th><th>Unidad</th><th>Capital usado</th><th>Entrada</th><th>Cierre</th><th>Resultado</th><th>%</th><th>Plan</th><th></th></tr></thead>
    <tbody>${trades.map(t => `<tr>
      ...
      <td>${n(t._planUnitResolved) || '—'} / ${plan.diasMes}</td>
...
function renderCleanProjection(plan, currentMonth) {
  const months = buildMonthlyFromPlan(plan);
  const start = Math.max(1, currentMonth || 1);
  const rows = months.slice(start - 1, Math.min(months.length, start + 4));
  return `<div class="card">
    <div class="alert-info"><strong>Proyección clara:</strong> esta vista separa lo real de lo proyectado. La meta del ciclo no baja si pierdes; lo que cambia es el rendimiento necesario para alcanzarla.</div>
    <div class="tbl-wrap"><table>
      <thead><tr><th>Ciclo</th><th>Capital inicial proyectado</th><th>Meta del ciclo</th><th>Ganancia requerida</th><th>Estado</th></tr></thead>
      <tbody>${rows.map(m => {
        const cs = cycleSummaryForPlan(plan, m.month);
        const isCurrent = m.month === currentMonth;
        return `<tr class="${isCurrent?'current-day':''}">
          <td><strong>${m.month}</strong></td>
          <td>${money(cs.startCapital)}</td>
          <td><strong>${money(cs.targetEnd)}</strong></td>
          <td>${money(cs.targetMoney)}</td>
          <td><span class="badge ${isCurrent?'badge-blue':'badge-gray'}">${isCurrent?'Actual':'Proyectado'}</span></td>
        </tr>`;
      }).join('')}</tbody>
    </table></div>
  </div>
  <div class="card">
    <div class="card-title">Gráficas del plan</div>
    <div class="chart-mode-row">
      ${chartBtn(plan.kind,'capital','Capital vs meta')}
      ${chartBtn(plan.kind,'pnl','P&L por operación')}
      ${chartBtn(plan.kind,'ocupado','Libre vs ocupado')}
      ${chartBtn(plan.kind,'rendimiento','Rendimiento %')}
      ${chartBtn(plan.kind,'necesario','% necesario')}
    </div>
    <div class="chart-wrap"><canvas id="plan-chart-${plan.kind}"></canvas></div>
    <p class="chart-note">Cambia la vista para entender si vas arriba/abajo del plan, cuánto capital está libre, qué rendimiento trae cada trade y qué porcentaje necesitarías para cerrar la meta.</p>
  </div>`;
}
function renderPlanClean(kind, ctx) {
  const { plan, month, day, curr, currStatus, week, cycle, dayTrades, allTrades, openTrades, closedAll, todayClosed, gainDay, lossDay, capitalConsolidado, capitalUsadoPlan, capitalLibrePlan, floatingTotal } = ctx;
  const cycleTrades = cycle?.summary?.trades || [];
  const cycleClosed = cycleTrades.filter(t => t.estado === 'Cerrada');
  const gainCycle = cycleClosed.filter(t=>n(t.pnl)>0).reduce((s,t)=>s+n(t.pnl),0);
  const lossCycle = cycleClosed.filter(t=>n(t.pnl)<0).reduce((s,t)=>s+n(t.pnl),0);
  const commCycle = cycleTrades.reduce((s,t)=>s+n(t.comision),0);
  const currentCapitalCycle = Math.max(0, n(cycle.startCapital) + n(cycle.consolidatedMoney));
  const targetEnd = n(cycle.targetEnd);
  const progress = targetEnd > 0 ? clampPct((currentCapitalCycle / targetEnd) * 100, 100) : 0;
  const progressRaw = targetEnd > 0 ? (currentCapitalCycle / targetEnd) * 100 : 0;
  const faltanteMeta = Math.max(0, targetEnd - currentCapitalCycle);
  const rendimientoCiclo = n(cycle.startCapital) > 0 ? n(cycle.consolidatedMoney) / n(cycle.startCapital) : 0;
  const tab = selectedTab(kind);
  const unitLabel = plan.unitLabel || 'Día';

  let tabContent = '';
  if (tab === 'semana') {
    tabContent = `
      <div class="kpi-grid clean-plan-grid">
        ${kpi('Ganancia semana', money(week.gains), 'Operaciones cerradas positivas', 'teal')}
        ${kpi('Pérdida semana', money(week.losses), 'Operaciones cerradas negativas', week.losses<0?'red':'teal')}
        ${kpi('Neto semana', money(week.pnl), `Meta ideal: ${money(week.idealGoal)}`, week.pnl>=0?'teal':'red')}
        ${kpi('Comisiones semana', '-'+money(week.commissions), 'Comisiones registradas', week.commissions>0?'red':'teal')}
      </div>
      <div class="card">${renderPlanTradesTable(plan, week.trades, 'Sin operaciones esta semana.')}</div>`;
  } else if (tab === 'dia') {
    tabContent = `
      <div class="kpi-grid clean-plan-grid">
        ...
        ${kpi('Neto del día', money(gainDay + lossDay), mxn(gainDay + lossDay), (gainDay+lossDay)>=0?'teal':'red')}
        ${kpi('Operaciones del día/unidad', String(dayTrades.length), `${unitLabel} ${day} / ${plan.diasMes}`, 'blue')}
      </div>
      <div class="card">${renderPlanTradesTable(plan, dayTrades, 'No hay operaciones en esta unidad.')}</div>`;
  } else if (tab === 'operaciones') {
    tabContent = `
      <div class="sec-title">Operaciones abiertas</div>
      <div class="card">${renderPlanTradesTable(plan, openTrades, 'Sin posiciones abiertas dentro del plan.')}</div>
      <div class="sec-title">Operaciones cerradas del ciclo</div>
      <div class="card">${renderPlanTradesTable(plan, cycleClosed, 'Sin operaciones cerradas en este ciclo.')}</div>`;
  } else if (tab === 'proyeccion') {
    tabContent = renderCleanProjection(plan, month);
  } else if (tab === 'captura') {
    tabContent = renderTradeCaptureAssistant(plan);
  } else {
    tabContent = `
      <div class="kpi-grid clean-plan-grid">
        ${kpi('Dinero inicial del ciclo', money(cycle.startCapital), 'Base fija del ciclo actual', 'blue')}
        ${kpi('Capital ocupado', money(capitalUsadoPlan), `${openTrades.length} posición(es) abierta(s) al costo`, capitalUsadoPlan>0?'amber':'teal')}
        ${kpi('P&L flotante', money(floatingTotal), 'No mueve la meta hasta cerrar', floatingTotal>=0?'teal':'red')}
        ${kpi('Capital libre', money(capitalLibrePlan), 'Disponible para nuevas entradas dentro del plan', capitalLibrePlan>0?'teal':'amber')}
        ${kpi('Comisiones del ciclo', '-'+money(commCycle), 'Comisiones registradas en el ciclo', commCycle>0?'red':'teal')}
        ${kpi('Ganancia ciclo', money(gainCycle), 'Ganancia cerrada del ciclo', 'teal')}
        ${kpi('Pérdida ciclo', money(lossCycle), 'Pérdida cerrada del ciclo', lossCycle<0?'red':'teal')}
        ${kpi('Rendimiento ciclo', pct(rendimientoCiclo), `${money(cycle.consolidatedMoney)} sobre ${money(cycle.startCapital)}`, rendimientoCiclo>=0?'teal':'red')}
        ${kpi('Avance ciclo', progressRaw.toFixed(1)+'%', `${money(currentCapitalCycle)} de ${money(targetEnd)}`, progressRaw>=100?'teal':'blue')}
        ${kpi('Faltante ciclo', money(faltanteMeta), faltanteMeta>0?'Para cerrar la meta del ciclo':'Meta alcanzada', faltanteMeta<=0?'teal':'amber')}
      </div>

      <div class="card">
        <div style="display:flex;justify-content:space-between;font-size:13px;margin-bottom:8px;gap:10px;flex-wrap:wrap">
          <span><strong>Avance real del ciclo</strong></span>
          <span>Capital real: <strong>${money(currentCapitalCycle)}</strong> / Meta: <strong>${money(targetEnd)}</strong></span>
        </div>
        <div class="prog-bar"><div class="prog-fill" style="width:${progress}%;background:${progress>=100?'var(--teal)':'var(--blue)'}"></div></div>
        <div class="prog-label">Avance ${progressRaw.toFixed(1)}% · Faltante: <strong>${money(faltanteMeta)}</strong> · Estado: <strong>${cycle.estado}</strong></div>
      </div>

      <div class="sec-title">${unitLabel} actual</div>
      <div class="card">
        <div class="simple-unit">
          <div><span>Unidad</span><strong>${curr.day} / ${plan.diasMes}</strong></div>
          <div><span>Capital inicial unidad</span><strong>${money(currStatus.actualBase)}</strong></div>
          <div><span>Capital usado</span><strong>${money(currStatus.used)}</strong></div>
          <div><span>Capital libre unidad</span><strong>${money(currStatus.remaining)}</strong></div>
      ...
      <div class="card">
        ${renderPlanTradesTable(plan, dayTrades, 'No hay trades en esta unidad.')}
  ...
  return `
    ...
    <div class="alert-info">
      <strong>${plan.label}:</strong> vista simplificada. La pantalla principal muestra solo el ciclo: dinero inicial, capital ocupado, P&L flotante, capital libre, comisiones, ganancia, pérdida, rendimiento, avance y faltante. El buying power manual queda fuera de esta vista para no contaminar los cálculos del plan.
    </div>

    <div class="plan-tabs">
      ${tabBtn(kind,'resumen','Resumen ciclo')}
      ${tabBtn(kind,'semana','Semana')}
      ${tabBtn(kind,'dia','Día / unidad')}
      ${tabBtn(kind,'operaciones','Operaciones')}
      ${tabBtn(kind,'proyeccion','Proyección')}
      ${tabBtn(kind,'captura','Captura / análisis')}
    </div>

    ${tabContent}

    <div class="alert-info">📌 Fechas blindadas como texto YYYY-MM-DD. Si una fila vieja venía como fecha de Google Sheets, EVA intenta mostrarla sin desfase al leerla.</div>`;
}
...
function effectivePlanInitialCapital(plan) {
  // v26.3: la Ruta de Libertad Financiera debe tomar el capital real del motor,
  // no un campo manual arrastrado. En opciones, si el plan quedó inflado por
  // configuración manual, usamos el primer trade real como base operativa.
  const configured = Math.max(0, n(plan.planCapital));
  const trades = unitTradesWithFallback(plan).filter(t => isCounted(t));
  const first = trades.find(t => n(t._planUnitResolved) === 1) || trades[0];
  const firstSub = first ? Math.max(0, n(first.subtotal)) : 0;
  if (!plan.isAcc && firstSub > 0 && (configured === 0 || configured > firstSub * 1.5)) return firstSub;
  return configured;
}
...
function isInitialSeedDoubleCount(plan, t) {
  // Si el primer trade del plan quedó marcado por error como "liberado",
  // no debemos sumar su principal otra vez. Ejemplo real: opciones inició con $364,
  // cerró con +$372 y el sistema lo estaba leyendo como $364 + $372 + $364 = $1,100.
  const unit = n(t?._planUnitResolved);
  if (unit !== 1) return false;
  const initial = effectivePlanInitialCapital(plan);
  const sub = n(t?.subtotal);
  if (!initial || !sub) return false;
  return Math.abs(sub - initial) <= Math.max(5, initial * 0.03);
}
function planReleasedPrincipal(plan, maxUnit = Infinity) {
  ...
  // v26.4: evita doble conteo del capital semilla/inicial transferido a otro motor.
  return unitTradesWithFallback(plan)
    .filter(t => isCounted(t) && t.estado === 'Cerrada' && isCapitalReleaseToPlan(t) && !isInitialSeedDoubleCount(plan, t) && n(t._planUnitResolved) > 0 && n(t._planUnitResolved) <= maxUnit)
...
function planConsolidatedCapital(plan, maxUnit = Infinity) {
  return Math.max(0, effectivePlanInitialCapital(plan) + planRealizedPnl(plan, maxUnit) + planReleasedPrincipal(plan, maxUnit));
}
...
function cycleSummaryForPlan(plan, month) {
  // v25.4: el ciclo operativo mide el avance consolidado del plan.
  // El flotante se informa aparte como estimación, pero NO cierra el ciclo ni mueve el compound.
  const unitStart = (month - 1) * plan.diasMes + 1;
  const unitEnd = month * plan.diasMes;
  const rows = buildDailyFromPlan(plan, month);
  const trades = unitTradesWithFallback(plan).filter(t => isCounted(t) && isIncludedInPlan(t) && n(t._planUnitResolved) >= unitStart && n(t._planUnitResolved) <= unitEnd);
  ...
  const consolidatedMoney = summary.hasData ? n(summary.pnl) : 0;      // Solo cerrado/integrado.
  const floatingMoney = summary.hasData ? n(summary.flotante) : 0;      // Temporal abierto.
  const estimatedMoney = consolidatedMoney + floatingMoney;             // Vista informativa.
  const realMoney = consolidatedMoney;
  const realPct = startCapital > 0 ? realMoney / startCapital : 0;
  const estimatedPct = startCapital > 0 ? estimatedMoney / startCapital : 0;
  const faltanteMoney = Math.max(0, targetMoney - realMoney);
  const faltantePct = Math.max(0, targetPct - realPct);
  const faltanteEstimadoMoney = Math.max(0, targetMoney - estimatedMoney);
  const faltanteEstimadoPct = Math.max(0, targetPct - estimatedPct);
  const targetReached = summary.hasData && realMoney >= targetMoney;
  const estimatedTargetReached = summary.hasData && estimatedMoney >= targetMoney;
  ...
  if (targetReached && !unitsComplete) {
    ...
    tone = 'teal';
    frase = 'Meta del ciclo alcanzada con ganancias cerradas. Puedes cerrar etapa o seguir con más riesgo.';
  ...
  } else if (!targetReached && estimatedTargetReached) {
    estado = 'Meta estimada con flotante';
    tone = 'amber';
    frase = 'El flotante sugiere que podrías alcanzar la meta, pero aún no está consolidada hasta cerrar.';
  } else if (unitsComplete && !targetReached) {
    estado = 'Unidades completas, meta pendiente';
    tone = 'red';
    frase = 'Terminaste las unidades, pero falta rendimiento cerrado para la meta ideal del ciclo.';
  } else if (realMoney > 0) {
    estado = 'Avanzando';
    tone = 'amber';
    frase = 'Vas avanzando; todavía falta para la meta ideal del ciclo.';
  } else if (estimatedMoney < 0) {
    estado = 'Debajo del plan';
    tone = 'red';
    frase = 'El ciclo va abajo considerando flotante. Cuida tamaño de posición y calidad de entrada.';
  }
  return { unitStart, unitEnd, completedUnits, openUnits, startCapital, targetEnd, minEnd, targetMoney, minMoney, targetPct, minPct, realMoney, realPct, consolidatedMoney, floatingMoney, estimatedMoney, estimatedPct, faltanteMoney, faltantePct, faltanteEstimadoMoney, faltanteEstimadoPct, targetReached, estimatedTargetReached, unitsComplete, estado, tone, frase, summary };
}
...
function renderPlan(kind) {
  ...
  // Libre operativo de la unidad actual: si capturaste buying power y abriste una operación,
  // este número debe mostrar lo que realmente queda para nuevas entradas en ESTA unidad.
  const capitalLibreUnidad = Math.max(0, n(currStatus.remaining));
  // Libre teórico total del plan: se conserva solo para cálculos internos, no como KPI principal.
  ...
  const disponibleRealCuenta = cuentaDisponibleReal();
  const buyingPowerText = disponibleRealCuenta == null ? 'No capturado' : money(disponibleRealCuenta);

  return renderPlanClean(kind, { plan, month, day, curr, currStatus, week, cycle, months, dayTrades, allTrades, openTrades, closedAll, todayClosed, gainDay, lossDay, usedTotal, floatingTotal, realizedTotal, capitalConsolidado, capitalUsadoPlan, capitalLibreUnidad, capitalLibrePlan, pendingRelease, pendingReleaseTrades, releasedIntegrated, unitLabel });
  ...
  return `
    ...
    <div class="alert-info">
      <strong>${plan.label}:</strong> combina operación real con interés compuesto. El plan avanza por <strong>${unitLabel}s</strong>. El saldo conciliado queda como referencia histórica; para operar se usa el <strong>Disponible real / buying power</strong> capturado manualmente. Las posiciones viejas abiertas se muestran como <strong>capital pendiente de liberar</strong> hasta que las cierres y decidas integrarlas.
    </div>

    <div class="kpi-grid compact-plan">
      ${kpi('Balance inicial del plan', money(plan.planCapital), 'Punto de partida del compound', 'blue')}
      ${kpi('Capital asignado al plan', money(capitalConsolidado), 'Balance inicial + P&L cerrado + capital liberado integrado', 'blue')}
      ${kpi('Capital en posiciones abiertas', money(capitalUsadoPlan), `${openTrades.length} posición(es) abierta(s) dentro del plan`, capitalUsadoPlan>0?'amber':'teal')}
      ${kpi('Capital libre para operar ahora', money(capitalLibreUnidad), `Libre de la unidad actual · Buying power capturado: ${buyingPowerText}`, capitalLibreUnidad>0?'teal':'amber')}
      ${kpi('Capital pendiente de liberar', money(pendingRelease), `${pendingReleaseTrades.length} posición(es) vieja(s) abierta(s) fuera del compound`, pendingRelease>0?'amber':'teal')}
      ...
      ${kpi('Ganancia/Pérdida flotante', money(floatingTotal), plan.isAcc ? 'Temporal de posiciones abiertas del plan' : 'Opciones sin cotización automática', floatingTotal>=0?'teal':'red')}
      ...
      ${kpi('Capital potencial si se libera pendiente', money(potencialSiLiberaEnMeta), `Pendiente ${money(pendingRelease)} + meta ${pct(plan.targetPct)}`, potencialSiLiberaEnMeta>=0?'teal':'amber')}
      ${kpi('Disponible real / buying power', buyingPowerText, 'Dato manual de cuenta; no modifica el plan', disponibleRealCuenta==null?'amber':'teal')}
      ${kpi('Ganancia del día', money(gainDay), mxn(gainDay), gainDay>=0?'teal':'red')}
      ${kpi('Pérdida del día', money(lossDay), mxn(lossDay), lossDay<0?'red':'teal')}
      ...
      ${kpi('Rendimiento promedio', pct(week.avgReturn), `${week.closed.length} cierre(s) esta semana`, week.avgReturn>=0?'teal':'red')}
    ...
    <div class="card cycle-card ${cycle.tone}">
      <div class="simple-unit">
        <div><span>Balance inicial del ciclo</span><strong>${money(cycle.startCapital)}</strong></div>
        <div><span>Resultado consolidado</span><strong class="${cycle.consolidatedMoney>=0?'green':'red'}">${money(cycle.consolidatedMoney)} / ${pct(cycle.realPct)}</strong></div>
        <div><span>Flotante abierto</span><strong class="${cycle.floatingMoney>=0?'green':'red'}">${money(cycle.floatingMoney)}</strong></div>
        <div><span>Resultado estimado</span><strong class="${cycle.estimatedMoney>=0?'green':'red'}">${money(cycle.estimatedMoney)} / ${pct(cycle.estimatedPct)}</strong></div>
        <div><span>Objetivo ideal</span><strong>${money(cycle.targetMoney)} / ${pct(cycle.targetPct)}</strong></div>
        <div><span>Faltante consolidado</span><strong class="${cycle.faltanteMoney<=0?'green':'red'}">${money(cycle.faltanteMoney)} / ${pct(cycle.faltantePct)}</strong></div>
        <div><span>Faltante estimado</span><strong class="${cycle.faltanteEstimadoMoney<=0?'green':'red'}">${money(cycle.faltanteEstimadoMoney)} / ${pct(cycle.faltanteEstimadoPct)}</strong></div>
    ...
    <div class="card">
      <div class="simple-unit">
        <div><span>Unidad</span><strong>${curr.day} / ${plan.diasMes}</strong></div>
        <div><span>Capital inicial de unidad</span><strong>${money(currStatus.actualBase)}</strong></div>
        <div><span>Capital usado</span><strong>${money(currStatus.used)}</strong></div>
        <div><span>Capital libre</span><strong>${money(currStatus.remaining)}</strong></div>
    ...
    <div class="month-summary-grid">
      ${months.map(m => { const ms = monthSummaryForPlan(plan, m.month); return `<button class="month-summary-card ${m.month===month?'active':''}" onclick="selectPlanMonth('${kind}',${m.month})">
        ...
        <span class="${n(ms.neto)>=0?'green':'red'}">Resultado estimado: ${renderPlanMoneyOrDash(ms.neto)} ${ms.hasData ? '('+pct(n(ms.neto)/n(ms.balanceInicial))+')' : ''}</span>
        ...
        <span class="${cycleSummaryForPlan(plan, m.month).faltanteMoney<=0?'green':'red'}">Faltante consolidado: ${pct(cycleSummaryForPlan(plan, m.month).faltantePct)} / ${money(cycleSummaryForPlan(plan, m.month).faltanteMoney)}</span>
...
function renderTradeCaptureAssistant(plan) {
  const saved = lastTradeImageData[plan.kind];
  return `<div class="card">
    <div class="card-title">Captura asistida de trade</div>
    <div class="alert-info"><strong>Versión práctica:</strong> sube una captura del ticket o del gráfico, EVA la usa como evidencia visual y te genera una ficha de análisis. La lectura automática/OCR completa requiere conectar una API de visión; por ahora no guarda nada sin que tú confirmes los datos.</div>
    <div class="form-grid two">
      <div class="form-group"><label>Subir captura</label><input type="file" accept="image/*" id="trade-shot-${plan.kind}" onchange="loadTradeScreenshot('${plan.kind}')"></div>
      <div class="form-group"><label>Ticker / contrato</label><input type="text" id="shot-symbol-${plan.kind}" placeholder="NVDA, SPY, SOXL..."></div>
      <div class="form-group"><label>Temporalidad</label><input type="text" id="shot-tf-${plan.kind}" placeholder="1H, 15M, diario..."></div>
      <div class="form-group"><label>Resultado del trade</label><select id="shot-result-${plan.kind}"><option value="ganancia">Ganancia</option><option value="perdida">Pérdida</option><option value="abierto">Sigue abierto</option></select></div>
    </div>
    <div id="shot-preview-${plan.kind}" class="shot-preview">${saved ? `<img src="${saved}" alt="Captura del trade">` : '<span>La captura aparecerá aquí.</span>'}</div>
    <div class="form-grid two">
      <div class="form-group"><label>Qué salió bien</label><textarea id="shot-good-${plan.kind}" rows="3" placeholder="Ej. respeté entrada, esperé confirmación, salí en zona..."></textarea></div>
      <div class="form-group"><label>Qué salió mal / duda</label><textarea id="shot-bad-${plan.kind}" rows="3" placeholder="Ej. entré tarde, stop lejos, vendí antes..."></textarea></div>
    </div>
    <button class="btn ${plan.isAcc?'btn-green':'btn-blue'}" onclick="buildTradeReview('${plan.kind}')">Generar ficha de análisis</button>
    <div id="trade-review-${plan.kind}" style="margin-top:14px"></div>
  </div>`;
}
function loadTradeScreenshot(kind) {
  const input = document.getElementById(`trade-shot-${kind}`);
  const file = input?.files?.[0];
  if (!file) return;
  const reader = new FileReader();
  reader.onload = e => {
    lastTradeImageData[kind] = e.target.result;
    const box = document.getElementById(`shot-preview-${kind}`);
    if (box) box.innerHTML = `<img src="${e.target.result}" alt="Captura del trade">`;
  };
  reader.readAsDataURL(file);
}
function buildTradeReview(kind) {
  const plan = planSettings(kind);
  const symbol = String(document.getElementById(`shot-symbol-${kind}`)?.value || '').trim().toUpperCase() || 'Sin ticker';
  const tf = String(document.getElementById(`shot-tf-${kind}`)?.value || '').trim() || 'Sin temporalidad';
  const result = document.getElementById(`shot-result-${kind}`)?.value || 'abierto';
  const good = String(document.getElementById(`shot-good-${kind}`)?.value || '').trim() || 'Pendiente de documentar.';
  const bad = String(document.getElementById(`shot-bad-${kind}`)?.value || '').trim() || 'Pendiente de documentar.';
  const cycle = cycleSummaryForPlan(plan, Math.max(1, Math.ceil(currentPlanUnit(plan) / plan.diasMes)));
  const currentCapital = Math.max(0, n(cycle.startCapital) + n(cycle.consolidatedMoney));
  const remainingUnits = Math.max(1, plan.diasMes - Math.min(plan.diasMes, cycle.completedUnits));
  const required = requiredReturnPerUnit(currentCapital, cycle.targetEnd, remainingUnits);
  const verdict = required <= plan.targetPct ? 'La meta sigue dentro de tu regla configurada.' : 'La meta exige más que tu regla actual; considera extensión o ajuste de riesgo.';
  const el = document.getElementById(`trade-review-${kind}`);
  if (!el) return;
  el.innerHTML = `<div class="trade-review-card">
    <div class="card-title">Ficha de análisis — ${esc(symbol)}</div>
    <div class="simple-unit">
      <div><span>Motor</span><strong>${esc(plan.label)}</strong></div>
      <div><span>Temporalidad</span><strong>${esc(tf)}</strong></div>
      <div><span>Resultado marcado</span><strong>${esc(result)}</strong></div>
      <div><span>Capital real ciclo</span><strong>${money(currentCapital)}</strong></div>
      <div><span>Faltante meta</span><strong>${money(Math.max(0, cycle.targetEnd-currentCapital))}</strong></div>
      <div><span>% necesario promedio</span><strong>${isFinite(required) ? pct(required) : '—'}</strong></div>
    </div>
    <div class="alert-info" style="margin-top:10px">${verdict}</div>
    <div class="review-cols">
      <div><strong>✅ Qué salió bien</strong><p>${esc(good)}</p></div>
      <div><strong>⚠️ Qué revisar</strong><p>${esc(bad)}</p></div>
    </div>
    <p class="chart-note">Esta ficha no sustituye el registro del trade. Sirve para revisar la ejecución y después decidir si capturas o corriges la operación.</p>
  </div>`;
}
function requiredReturnPerUnit(currentCapital, targetCapital, remainingUnits) {
  const c = Math.max(0, n(currentCapital));
  const t = Math.max(0, n(targetCapital));
  const r = Math.max(1, Math.round(n(remainingUnits) || 1));
  if (!c || c >= t) return 0;
  return Math.pow(t / c, 1 / r) - 1;
}
function drawPlanChart(kind) {
  ...
  const mode = selectedPlanChart[kind] || 'capital';
  const totalUnits = Math.min(plan.diasMes * plan.meses, Math.max(plan.diasMes, currentPlanUnit(plan) + 8));
  const labels = [];

  if (mode === 'pnl') {
    const trades = unitTradesWithFallback(plan).filter(t => isCounted(t) && isIncludedInPlan(t) && t.estado === 'Cerrada');
    const rows = trades.slice(-30).map(t => ({ label: `${n(t._planUnitResolved)||'—'} · ${tradeSymbol(t, plan.isAcc)}`, pnl: n(t.pnl) }));
    chartInstances[kind] = new Chart(canvas, {
      type: 'bar',
      data: { labels: rows.map(r=>r.label), datasets: [{ label:'P&L cerrado', data: rows.map(r=>+r.pnl.toFixed(2)), backgroundColor: rows.map(r=>r.pnl>=0?'rgba(29,158,117,.55)':'rgba(184,64,64,.55)') }] },
      options: chartOptionsMoney('Operación', 'P&L')
    });
    return;
  }
  ...
  if (mode === 'rendimiento') {
    const trades = unitTradesWithFallback(plan).filter(t => isCounted(t) && isIncludedInPlan(t) && t.estado === 'Cerrada');
    const rows = trades.slice(-30).map(t => ({ label: `${n(t._planUnitResolved)||'—'} · ${tradeSymbol(t, plan.isAcc)}`, ret: n(tradeReturnPct(t))*100 }));
    chartInstances[kind] = new Chart(canvas, {
      type: 'bar',
      data: { labels: rows.map(r=>r.label), datasets: [{ label:'Rendimiento por trade', data: rows.map(r=>+r.ret.toFixed(2)), backgroundColor: rows.map(r=>r.ret>=0?'rgba(29,158,117,.55)':'rgba(184,64,64,.55)') }] },
      options: chartOptionsPct('Operación', 'Rendimiento')
    });
    return;
  }

  const ideal = [], minimo = [], real = [], libre = [], ocupado = [], necesario = [];
  let minCap = effectivePlanInitialCapital(plan);
  let idealCap = effectivePlanInitialCapital(plan);
  const selectedMonth = selectedPlanMonth[kind] || Math.max(1, Math.ceil(currentPlanUnit(plan) / plan.diasMes));
  const cycle = cycleSummaryForPlan(plan, selectedMonth);
  for (let u=1; u<=totalUnits; u++) {
    ...
    const st = statusForDay(plan, rr);
    const realCap = +(st.actualBase + st.real).toFixed(2);
    real.push(realCap);
    libre.push(+Math.max(0, st.remaining).toFixed(2));
    ocupado.push(+Math.max(0, st.used).toFixed(2));
    const rem = Math.max(1, plan.diasMes - (((u-1) % plan.diasMes) + 1));
    necesario.push(+(requiredReturnPerUnit(realCap, cycle.targetEnd, rem) * 100).toFixed(2));
  ...
  if (mode === 'ocupado') {
    chartInstances[kind] = new Chart(canvas, {
      ...
      data: { labels, datasets: [
        { label: 'Capital libre', data: libre, backgroundColor: 'rgba(29,158,117,.55)' },
        { label: 'Capital ocupado', data: ocupado, backgroundColor: 'rgba(186,117,23,.55)' }
      ]},
      options: { ...chartOptionsMoney('Unidad operativa', 'Capital'), scales: { x: { stacked:true, title:{display:true,text:'Unidad operativa'}, grid:{display:false}}, y: { stacked:true, ticks:{ callback:v=>money(v)}, grid:{color:'rgba(0,0,0,.06)'} } } }
    });
    return;
  }
  ...
  if (mode === 'necesario') {
    chartInstances[kind] = new Chart(canvas, {
      type: 'line',
      data: { labels, datasets: [
        { label: '% necesario promedio', data: necesario, borderColor:'#BA7517', borderWidth:2.5, pointRadius:2, fill:false, tension:.25 },
        { label: 'Regla máxima configurada', data: labels.map(()=>plan.targetPct*100), borderColor:'#B84040', borderWidth:2, pointRadius:0, fill:false, tension:0 }
      ]},
      options: chartOptionsPct('Unidad operativa', 'Rendimiento necesario')
    });
    return;
  }
  ...
  chartInstances[kind] = new Chart(canvas, {
    ...
    options: chartOptionsMoney('Unidad operativa', 'Capital')
  });
}
function chartOptionsMoney(xLabel, yLabel) {
  return { responsive:true, maintainAspectRatio:false,
    plugins:{ legend:{ position:'top' }, tooltip:{ callbacks:{ label:ctx => ctx.dataset.label + ': ' + money(ctx.parsed.y) } } },
    scales:{ x:{ title:{display:true,text:xLabel}, grid:{display:false}}, y:{ title:{display:false,text:yLabel}, ticks:{callback:v=>money(v)}, grid:{color:'rgba(0,0,0,.06)'} } }
  };
}
function chartOptionsPct(xLabel, yLabel) {
  return { responsive:true, maintainAspectRatio:false,
    plugins:{ legend:{ position:'top' }, tooltip:{ callbacks:{ label:ctx => ctx.dataset.label + ': ' + Number(ctx.parsed.y).toFixed(2) + '%' } } },
    scales:{ x:{ title:{display:true,text:xLabel}, grid:{display:false}}, y:{ ticks:{callback:v=>Number(v).toFixed(0)+'%'}, grid:{color:'rgba(0,0,0,.06)'} } }
  };
}
...
function renderCuentas() {
  ...
  return `
    ...
    <div class="kpi-grid">
      ${kpi('Saldo conciliado histórico', money(total), mxn(total), 'blue')}
      ${kpi('Cuentas activas', String(cuentas.filter(c => String(c.activa||'SI').toUpperCase() !== 'NO').length), '', 'blue')}
    </div>
    <div class="btn-row"><button class="btn btn-green" onclick="openModal('nueva-cuenta')">+ Nueva cuenta/subcuenta</button><button class="btn btn-blue" onclick="openModal('actualizar-disponible-real')">💵 Actualizar disponible real</button></div>
    <div class="card">
      <div class="tbl-wrap"><table>
        <thead><tr><th>Cuenta</th><th>Tipo</th><th>Padre</th><th>Estrategia</th><th>Moneda</th><th>Saldo conciliado histórico</th><th>Ocupado</th><th>Disponible real</th><th>Notas</th><th></th></tr></thead>
...
// ── Ruta de Libertad Financiera ───────────────────────────────
function libertadSummaryCard(compact = false) {
  const f = getFreedomState();
  const retiroMsg = !f.puedeRetirar
    ? `No es posible retirar todavía. Primero llega a ${money(f.capitalProtegidoUSD)} combinados.`
    : (f.capitalOperativo < 50000
      ? `Puedes retirar mínimo ${money(f.retiroMensualUSD)} si el rendimiento del periodo lo cubre sin bajar de ${money(f.capitalProtegidoUSD)}.`
      : `Ya estás en zona de retiro controlado. Simula antes de sacar capital.`);
  const tone = !f.puedeRetirar ? 'amber' : (f.tocaBase ? 'red' : 'teal');
  return `<div class="card">
    <div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start;flex-wrap:wrap">
      <div>
        <div class="card-title">🏁 Ruta de Libertad Financiera</div>
        <div style="font-size:13px;color:var(--muted)">Capital operativo combinado = Acciones + Opciones, calculado desde tus planes reales; no depende del buying power manual.</div>
      </div>
      <button class="btn btn-blue" onclick="showPage('libertad')">Ver ruta completa</button>
    </div>
    <div class="kpi-grid" style="margin-top:12px">
      ${kpi('Capital operativo combinado', money(f.capitalOperativo), `Acciones ${money(f.accOperativo)} + Opciones ${money(f.opcOperativo)}`, 'blue')}
      ${kpi('Capital libre para nuevas entradas', money(f.capitalLibrePlanes), `Acciones ${money(f.accLibre)} + Opciones ${money(f.opcLibre)}`, f.capitalLibrePlanes>0?'teal':'amber')}
      ${kpi('Meta etapa actual', money(f.stage.target), `Avance ${(f.stageProgress*100).toFixed(1)}% · faltan ${money(f.faltanteEtapa)}`, f.stageProgress>=1?'teal':'amber')}
      ${kpi('Retiro configurado', money(f.retiroMensualUSD), retiroMsg, tone)}
    </div>
    <div class="prog-wrap"><div class="prog-bar"><div class="prog-fill" style="width:${Math.min(100, f.stageProgress*100).toFixed(1)}%;background:var(--teal)"></div></div><div class="prog-label">Avance hacia ${money(f.stage.target)}: ${(f.stageProgress*100).toFixed(1)}% · faltan ${money(f.faltanteEtapa)}</div></div>
  </div>`;
}
function renderLibertad() {
  const f = getFreedomState();
  const retiroStatus = !f.puedeRetirar ? 'Bloqueado' : (f.tocaBase ? 'Toca capital base' : 'Permitido bajo regla');
  const retiroTone = !f.puedeRetirar ? 'amber' : (f.tocaBase ? 'red' : 'teal');
  const msgSeed = Math.round(f.capitalOperativo + f.ingresoMensualUSD + new Date().getDate());
  const escenarios = [0.03, 0.05, 0.08, f.rendimientoMensual, 0.20]
    .filter((v,i,a)=>a.findIndex(x=>Math.abs(x-v)<0.0001)===i)
    .sort((a,b)=>a-b);
  return `
    <div class="alert-info"><strong>Regla de libertad:</strong> no se permite retiro antes de ${money(f.capitalProtegidoUSD)} combinados entre acciones y opciones. Después puedes simular retiro fijo; EVA calcula cuánto necesitas producir para no descompensar tu capital base.</div>
    <div class="btn-row">
      <button class="btn btn-blue" onclick="openModal('libertad-config')">⚙ Configurar ruta</button>
      <button class="btn" onclick="openModal('actualizar-disponible-real')">💵 Actualizar buying power</button>
    </div>
    <div class="kpi-grid">
      ${kpi('Capital operativo combinado', money(f.capitalOperativo), `Acciones ${money(f.accOperativo)} + Opciones ${money(f.opcOperativo)}`, 'blue')}
      ${kpi('Capital libre para nuevas entradas', money(f.capitalLibrePlanes), `Acciones ${money(f.accLibre)} + Opciones ${money(f.opcLibre)}`, f.capitalLibrePlanes>0?'teal':'amber')}
      ${kpi('Capital pendiente de liberar', money(f.capitalPendiente), `Acciones ${money(f.accPendiente)} + Opciones ${money(f.opcPendiente)}`, f.capitalPendiente>0?'amber':'teal')}
      ${kpi('Meta etapa actual', money(f.stage.target), `Avance ${(f.stageProgress*100).toFixed(1)}% · faltan ${money(f.faltanteEtapa)}`, f.stageProgress>=1?'teal':'amber')}
      ${kpi('Capital protegido', money(f.capitalProtegidoUSD), f.puedeRetirar ? 'Protección alcanzada' : 'Aún no se puede retirar', f.puedeRetirar?'teal':'amber')}
      ${kpi('Ingreso potencial mensual', money(f.ingresoMensualUSD), `${mxn(f.ingresoMensualUSD)} al ${(f.rendimientoMensual*100).toFixed(1)}%`, 'teal')}
      ${kpi('Retiro fijo simulado', money(f.retiroMensualUSD), `Necesitas ${(f.rendimientoNecesarioRetiro*100).toFixed(2)}% mensual para cubrirlo`, retiroTone)}
      ${kpi('Capital después de retiro', f.puedeRetirar ? money(f.capitalDespuesRetiro) : 'No permitido', retiroStatus, retiroTone)}
      ${kpi('Vivir tranquilo', money(f.gastoTranquiloUSD), `${money(f.gastoTranquiloMXN)} MXN · cubres ${(f.coberturaTranquilo*100).toFixed(1)}%`, 'blue')}
      ${kpi('Vivir muy bien', money(f.gastoIdealUSD), `${money(f.gastoIdealMXN)} MXN · cubres ${(f.coberturaIdeal*100).toFixed(1)}%`, 'blue')}
    </div>
    <div class="sec-title">Capital por motor</div>
    <div class="card">
      <div class="tbl-wrap"><table>
        <thead><tr><th>Motor</th><th>Capital consolidado</th><th>Pendiente de liberar</th><th>Capital operativo</th><th>Libre para nueva entrada</th></tr></thead>
        <tbody>
          <tr><td><strong>Acciones</strong></td><td>${money(f.accConsolidado)}</td><td>${money(f.accPendiente)}</td><td>${money(f.accOperativo)}</td><td>${money(f.accLibre)}</td></tr>
          <tr><td><strong>Opciones</strong></td><td>${money(f.opcConsolidado)}</td><td>${money(f.opcPendiente)}</td><td>${money(f.opcOperativo)}</td><td>${money(f.opcLibre)}</td></tr>
          <tr><td><strong>Total</strong></td><td>${money(f.capitalConsolidado)}</td><td>${money(f.capitalPendiente)}</td><td>${money(f.capitalOperativo)}</td><td>${money(f.capitalLibrePlanes)}</td></tr>
        </tbody>
      </table></div>
      <div class="alert-info" style="margin-top:10px">Estos valores salen de tus trades y planes. El buying power manual queda como corte de Charles; la Ruta usa el capital real calculado desde Acciones y Opciones. El capital libre es solo para nuevas entradas; el capital operativo es el que cuenta para metas.</div>
    </div>

    <div class="card">
      <div class="card-title">Mensaje EVA</div>
      <p style="font-size:15px;line-height:1.55">${esc(freedomMotivation(msgSeed))}</p>
      <div class="prog-wrap"><div class="prog-bar"><div class="prog-fill" style="width:${Math.min(100, f.stageProgress*100).toFixed(1)}%;background:var(--teal)"></div></div><div class="prog-label">Meta actual ${money(f.stage.target)} · avance ${(f.stageProgress*100).toFixed(1)}% · faltan ${money(f.faltanteEtapa)}</div></div>
    </div>
    <div class="sec-title">Simulador de retiro fijo</div>
    <div class="card">
      <div class="simple-unit">
        <div><span>Capital actual</span><strong>${money(f.capitalOperativo)}</strong></div>
        <div><span>Rendimiento configurado</span><strong>${(f.rendimientoMensual*100).toFixed(2)}%</strong></div>
        <div><span>Ganancia estimada</span><strong>${money(f.ingresoMensualUSD)}</strong></div>
        <div><span>Retiro</span><strong>${money(f.retiroMensualUSD)}</strong></div>
        <div><span>Capital final estimado</span><strong>${f.puedeRetirar ? money(f.capitalDespuesRetiro) : 'Bloqueado'}</strong></div>
      </div>
      <div class="alert-${f.tocaBase || !f.puedeRetirar ? 'warn' : 'info'}" style="margin-top:12px">
        ${!f.puedeRetirar ? `No es posible retirar: primero debes llegar a ${money(f.capitalProtegidoUSD)} combinados.` : (f.tocaBase ? `Este retiro toca tu capital base. Para retirar ${money(f.retiroMensualUSD)} necesitas producir al menos ${money(f.retiroMensualUSD)} (${(f.rendimientoNecesarioRetiro*100).toFixed(2)}%) y además respetar el capital protegido.` : `Retiro cubierto bajo la simulación. Después del retiro quedarías con ${money(f.capitalDespuesRetiro)}.`)}
      </div>
    </div>
    <div class="sec-title">Escenarios de ingreso mensual</div>
    <div class="card">
      <div class="tbl-wrap"><table><thead><tr><th>Rendimiento mensual</th><th>Ingreso USD</th><th>Ingreso MXN</th><th>Cobertura tranquilo</th><th>Cobertura vivir bien</th></tr></thead><tbody>
        ${escenarios.map(r => { const usd=f.capitalOperativo*r; return `<tr><td><strong>${(r*100).toFixed(1)}%</strong></td><td>${money(usd)}</td><td>${mxn(usd)}</td><td>${(f.gastoTranquiloUSD?usd/f.gastoTranquiloUSD*100:0).toFixed(1)}%</td><td>${(f.gastoIdealUSD?usd/f.gastoIdealUSD*100:0).toFixed(1)}%</td></tr>`; }).join('')}
      </tbody></table></div>
    </div>
    <div class="sec-title">Gráficas de progreso</div>
    <div class="card"><div class="card-title">Capital proyectado vs meta</div><div class="chart-wrap"><canvas id="libertad-capital-chart"></canvas></div></div>
    <div class="card"><div class="card-title">Ingreso potencial por etapa</div><div class="chart-wrap"><canvas id="libertad-income-chart"></canvas></div></div>
  `;
}
function drawLibertadCharts() {
  if (typeof Chart === 'undefined') {
    console.warn('Chart.js no cargó; se omiten gráficas de Libertad Financiera.');
    return;
  }
  const f = getFreedomState();
  const capCanvas = document.getElementById('libertad-capital-chart');
  const incCanvas = document.getElementById('libertad-income-chart');
  if (chartInstances.libCap) { try { chartInstances.libCap.destroy(); } catch(e){} }
  if (chartInstances.libInc) { try { chartInstances.libInc.destroy(); } catch(e){} }
  if (capCanvas) {
    const labels = [];
    const capitalData = [];
    const targetData = [];
    let cap = f.capitalOperativo;
    for (let m=0; m<=f.mesesProyeccion; m++) {
      labels.push(m===0?'Hoy':`Mes ${m}`);
      capitalData.push(+cap.toFixed(2));
      targetData.push(f.stage.target);
      if (m < f.mesesProyeccion) {
        const gain = cap * f.rendimientoMensual;
        const canWithdraw = cap >= f.capitalProtegidoUSD && (cap + gain - f.retiroMensualUSD) >= f.capitalProtegidoUSD;
        cap = cap + gain - (canWithdraw ? f.retiroMensualUSD : 0);
      }
    }
    chartInstances.libCap = new Chart(capCanvas, { type:'line', data:{ labels, datasets:[
      { label:'Capital proyectado', data:capitalData, tension:.25 },
      { label:'Meta etapa', data:targetData, tension:.1 }
    ]}, options:{ responsive:true, maintainAspectRatio:false, plugins:{ tooltip:{ callbacks:{ label:ctx => `${ctx.dataset.label}: ${money(ctx.parsed.y)}` } } }, scales:{ y:{ ticks:{ callback:v=>money(v) }, grid:{ color:'rgba(0,0,0,.06)' } }, x:{ grid:{ display:false } } } } });
  }
  if (incCanvas) {
    const stages = [10000,25000,50000,100000,250000,500000];
    chartInstances.libInc = new Chart(incCanvas, { type:'bar', data:{ labels:stages.map(x=>money(x)), datasets:[
      { label:`Ingreso al ${(f.rendimientoMensual*100).toFixed(1)}% mensual`, data: stages.map(x => +(x*f.rendimientoMensual*FX).toFixed(2)), backgroundColor:'#1D9E75', borderRadius:6 },
      { label:'Gasto tranquilo', data: stages.map(()=>f.gastoTranquiloMXN), backgroundColor:'#185FA5', borderRadius:6 }
    ]}, options:{ responsive:true, maintainAspectRatio:false, plugins:{ tooltip:{ callbacks:{ label:ctx => `${ctx.dataset.label}: ${money(ctx.parsed.y)} MXN` } } }, scales:{ y:{ ticks:{ callback:v=>money(v)+' MXN' }, grid:{ color:'rgba(0,0,0,.06)' } }, x:{ grid:{ display:false } } } } });
  }
}
...
function openModal(type) {
  ...
  if (type === 'config') {
    ...
    box.innerHTML = `<div class="modal-title">⚙ Configuración de EVA y planes</div>
      ...
      <div class="form-grid">
        ...
        <div class="form-group"><label>Fecha inicio plan acciones</label><input type="date" id="m-cfg-plan-inicio-acc" value="${dateInputValue(c.planInicioAcc)}"></div>
        <div class="form-group"><label>Fecha inicio plan opciones</label><input type="date" id="m-cfg-plan-inicio-opc" value="${dateInputValue(c.planInicioOpc)}"></div>
      ...
      <div class="alert-info" style="margin-top:10px"><strong>Libertad financiera:</strong> estos datos también se pueden ajustar desde la pestaña Ruta de Libertad Financiera.</div>
      <div class="modal-footer">
  ...
  if (type === 'libertad-config') {
    const f = freedomCfg();
    box.innerHTML = `<div class="modal-title">🏁 Configurar Ruta de Libertad Financiera</div>
      <div class="alert-info">Configura manualmente el rendimiento mensual, retiro fijo y capital protegido. EVA no permitirá retiros antes de llegar al capital protegido combinado.</div>
      <div class="form-grid">
        <div class="form-group"><label>Gasto mensual tranquilo (MXN)</label><input type="number" id="m-lib-gasto-tranquilo" value="${f.gastoTranquiloMXN}" step="100"></div>
        <div class="form-group"><label>Gasto mensual vivir muy bien (MXN)</label><input type="number" id="m-lib-gasto-ideal" value="${f.gastoIdealMXN}" step="100"></div>
        <div class="form-group"><label>% mensual proyectado</label><input type="number" id="m-lib-rend" value="${(f.rendimientoMensual*100).toFixed(2)}" step="0.1"></div>
        <div class="form-group"><label>Retiro mensual fijo (USD)</label><input type="number" id="m-lib-retiro" value="${f.retiroMensualUSD}" step="50"></div>
        <div class="form-group"><label>Capital protegido mínimo (USD)</label><input type="number" id="m-lib-protegido" value="${f.capitalProtegidoUSD}" step="100"></div>
        <div class="form-group"><label>Meta etapa actual (USD)</label><input type="number" id="m-lib-meta" value="${f.metaEtapaUSD}" step="100"></div>
        <div class="form-group"><label>Meta superior / retiro amplio (USD)</label><input type="number" id="m-lib-meta-amplia" value="${f.metaAmpliaUSD}" step="1000"></div>
        <div class="form-group"><label>Meses a proyectar</label><input type="number" id="m-lib-meses" value="${f.mesesProyeccion}" min="3" max="120"></div>
      </div>
      <div class="modal-footer">
        <button class="btn btn-green" onclick="saveLibertadConfig()">Guardar ruta</button>
        <button class="btn" onclick="closeModal()">Cancelar</button>
      </div>`;
    return;
  }
  ...
  if (type === 'nueva-pos-accion') {
    ...
    box.innerHTML = `<div class="modal-title">+ Nueva posición — Acción</div>
      ...
      <div class="form-grid">
        ...
        <div class="form-group"><label>Fecha apertura</label><input type="date" id="m-fecha" value="${dateInputValue(ctx ? ctx.date : todayISO)}"></div>
  ...
  if (type === 'cerrar-pos') {
    ...
    box.innerHTML = `<div class="modal-title">✕ Cerrar posición</div>
      ${all.length ? `<div class="form-grid">
        ...
        <div class="form-group"><label>Fecha cierre</label><input type="date" id="m-close-date" value="${dateInputValue(todayISO)}"></div>
  ...
  if (type === 'actualizar-disponible-real') {
    const cuenta = getCuentaByName(accountNameDefault()) || (D.cuentas && D.cuentas[0]) || {};
    box.innerHTML = `<div class="modal-title">💵 Actualizar disponible real</div>
      <div class="alert-info">Este dato debe venir de Charles como <strong>buying power / cash disponible</strong>. No toca el saldo conciliado histórico, no borra trades y no cambia el capital asignado al plan.</div>
      <div class="form-grid">
        <div class="form-group"><label>Cuenta</label><select id="m-real-cuenta">${cuentaOptions(cuenta.nombre || accountNameDefault())}</select></div>
        <div class="form-group"><label>Disponible real actual ($)</label><input type="number" id="m-real-disponible" step="0.01" value="${n(cuenta.disponible)}" placeholder="951.00"></div>
      </div>
      <div class="modal-footer">
        <button class="btn btn-green" onclick="doUpdateDisponibleReal()">Guardar disponible real</button>
        <button class="btn" onclick="closeModal()">Cancelar</button>
      </div>`;
    return;
  }
  ...
  if (type === 'nueva-cuenta') {
    box.innerHTML = `<div class="modal-title">+ Nueva cuenta/subcuenta</div>
      ...
      <div class="form-grid">
        ...
        <div class="form-group"><label>Saldo conciliado histórico ($)</label><input type="number" id="m-cuenta-saldo" step="0.01" value="0"></div>
        <div class="form-group"><label>Disponible real / buying power ($)</label><input type="number" id="m-cuenta-disponible" step="0.01" value="0"></div>
  ...
  if (type === 'nuevo-socio') {
    box.innerHTML = `<div class="modal-title">+ Agregar inversionista</div>
      <div class="form-grid">
        ...
        <div class="form-group"><label>Fecha ingreso</label><input type="date" id="m-socio-fecha" value="${dateInputValue(todayISO)}"></div>
  ...
  if (type === 'estado-cuenta') {
    const c = cfg();
    box.innerHTML = `<div class="modal-title">📥 Actualizar capital/configuración</div>
      <div class="alert-info">Usa esto para capital/configuración general. Para el buying power actual usa “Actualizar disponible real”. Para historial de trades usa Conciliación Bancaria.</div>
      <div class="form-grid">
  ...
  if (kind === 'acc') {
    box.innerHTML = `<div class="modal-title">✏ Editar acción — ${esc(item.ticker)}</div>
      <div class="form-grid">
        ...
        <div class="form-group"><label>Fecha compra</label><input type="date" id="e-fecha-compra" value="${dateInputValue(item.fechaCompra, '')}"></div>
        <div class="form-group"><label>Fecha cierre</label><input type="date" id="e-fecha-cierre" value="${dateInputValue(item.fechaCierre, '')}"></div>
    ...
    box.innerHTML = `<div class="modal-title">✏ Editar opción — ${esc(item.empresa)}</div>
      <div class="form-grid">
        ...
        <div class="form-group"><label>Comisión total ($)</label><input type="number" id="e-comision" value="${item.comision}" step="0.01"></div>
        <div class="form-group"><label>Fecha apertura</label><input type="date" id="e-fecha-apertura" value="${dateInputValue(item.fechaApertura, '')}"></div>
        <div class="form-group"><label>Fecha cierre</label><input type="date" id="e-fecha-cierre" value="${dateInputValue(item.fechaCierre, '')}"></div>
        <div class="form-group"><label>Fecha vencimiento</label><input type="date" id="e-fecha-venc" value="${dateInputValue(item.fechaVencim, '')}"></div>
        <div class="form-group"><label>Cuenta/Subcuenta</label><select id="e-cuenta">${cuentaOptions(item.accountName || item.cuenta || '')}</select></div>
        ${countSelect}
      </div>
      <div class="modal-footer">
...
async function doUpdateDisponibleReal() {
  const nombre = $('m-real-cuenta')?.value || accountNameDefault();
  const disponible = n($('m-real-disponible')?.value);
  const cuenta = getCuentaByName(nombre) || {};
  if (!nombre) { showToast('Elige una cuenta.', 'error'); return; }
  try {
    await api('updateCuentaByName', {
      nombre,
      tipo: cuenta.tipo || 'Broker',
      moneda: cuenta.moneda || 'USD',
      cuentaPadre: cuenta.cuentaPadre || '',
      estrategia: cuenta.estrategia || 'Mixta',
      saldoConciliado: n(cuenta.saldoConciliado),
      capitalOcupado: n(cuenta.capitalOcupado),
      disponible,
      notas: cuenta.notas || 'Disponible real actualizado manualmente desde EVA',
      activa: cuenta.activa || 'SI'
    });
    closeModal();
    await syncAll();
    showToast('Disponible real actualizado. Este dato se usa como buying power; no modifica trades ni el plan.', 'success');
  } catch(e) { showToast(e.message || 'No se pudo actualizar el disponible real.', 'error'); }
}
...
function openEditCuenta(rowIndex) {
  ...
  box.innerHTML = `<div class="modal-title">✏ Editar cuenta/subcuenta</div>
    <div class="form-grid">
      ...
      <div class="form-group"><label>Saldo conciliado histórico ($)</label><input type="number" id="m-cuenta-saldo" step="0.01" value="${n(c.saldoConciliado)}"></div>
      <div class="form-group"><label>Disponible real / buying power ($)</label><input type="number" id="m-cuenta-disponible" step="0.01" value="${n(c.disponible)}"></div>
...
async function doAddCuenta() {
  ...
  const payload = {
    ...
    disponible: n($('m-cuenta-disponible')?.value),
...
function openPlanConfig(kind) {
  ...
  box.innerHTML = `<div class="modal-title">⚙ Configurar plan de ${plan.label}</div>
    ...
    <div class="form-grid">
      ...
      <div class="form-group"><label>Fecha de inicio</label><input type="date" id="pc-inicio" value="${dateInputValue(plan.startDate)}"></div>
...
async function saveLibertadConfig() {
  const payload = {
    libertadGastoTranquiloMXN: n($('m-lib-gasto-tranquilo')?.value),
    libertadGastoIdealMXN: n($('m-lib-gasto-ideal')?.value),
    libertadRendimientoMensual: n($('m-lib-rend')?.value) / 100,
    libertadRetiroMensualUSD: n($('m-lib-retiro')?.value),
    libertadCapitalProtegidoUSD: n($('m-lib-protegido')?.value),
    libertadMetaEtapaUSD: n($('m-lib-meta')?.value),
    libertadMetaAmpliaUSD: n($('m-lib-meta-amplia')?.value),
    libertadMesesProyeccion: Math.max(3, Math.min(120, Math.round(n($('m-lib-meses')?.value) || 24)))
  };
  if (payload.libertadRendimientoMensual < 0) { showToast('El rendimiento mensual no puede ser negativo.', 'error'); return; }
  try {
    localStorage.setItem('eva_libertad_config', JSON.stringify(payload));
    D.config = { ...D.config, ...payload };
    closeModal();
    renderAll();
    await api('saveConfig', payload);
    // No hacemos sync inmediato para evitar que una lectura vieja pise el valor recién guardado.
    showToast(`Ruta guardada. Rendimiento mensual: ${(payload.libertadRendimientoMensual*100).toFixed(2)}%.`, 'success');
  } catch(e) { showToast(e.message, 'error'); }
}
...
// Exponer navegación para botones inline en GitHub Pages
try { window.showPage = showPage; window.openModal = openModal; } catch(e) {}

console.info('EVA build', EVA_BUILD_VERSION);
