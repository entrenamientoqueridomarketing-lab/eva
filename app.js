...
//        plan opciones por operación, proyectado vs real, edición de trades
// ============================================================
  ...
  return `
    ...
    <div class="plan-month-grid">
      ${months.map(m => `<button class="month-card ${m.month===month?'active':''}" onclick="selectPlanMonth('${kind}',${m.month})">
        ...
        <small>Mínimo: ${money(m.endMin)} · Ideal +${money(m.gain)} (${(m.pct*100).toFixed(1)}%)</small>
      </button>`).join('')}
    </div>
    ...
    <div class="card">
      <div class="tbl-wrap"><table class="plan-day-table">
        <thead><tr><th>${plan.unitLabel}</th><th>${plan.isAcc?'Fecha':'Fecha estimada'}</th><th>Proy. mínimo</th><th>Proy. ideal</th><th>Real antes</th><th>Operable</th><th>Usado</th><th>Libre</th><th>Meta mín.</th><th>Meta ideal</th><th>Real</th><th>Real vs ideal</th><th>Estado</th><th></th></tr></thead>
        <tbody>${daily.map(r => { const st = statusForDay(plan, r); const diff = st.actualBase - r.start; return `<tr class="${r.day===day?'current-day':''}">
          ...
          <td class="${diff>=0?'green':'red'}"><strong>${money(diff)}</strong></td>
