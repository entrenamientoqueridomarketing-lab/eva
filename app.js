// ============================================================
// EVA — app.js v4 final
// Fixes: setup con nombre, capital+socio, plan diario correcto,
//        gráficas, edición de trades, importador CSV Schwab
// ============================================================
...
function cfg() {
  const c = D.config;
  return {
    nombre: c.nombre || 'Trader',
    capAcc: n(c.capAcciones || c.capitalAcciones),
    capOpc: n(c.capOpciones || c.capitalOpciones),
    metaAcc: n(c.metaAcciones || 0.03),
    metaOpc: n(c.metaOpciones || 0.20),
    meses:   n(c.meses || 12),
    diasMes: n(c.diasMes || 20),
    usoAcc:  n(c.usoCapitalAcciones != null ? c.usoCapitalAcciones : 1),
    usoOpc:  n(c.usoCapitalOpciones != null ? c.usoCapitalOpciones : 0.33),
    fx:      n(c.tipoCambio || FX),
  };
}
...
function calcA(a) {
  ...
  return { ...a, acciones: acc, precioCompra: cp, comision: com, subtotal: sub, ingresosVenta: ing, estado, pnl: pnlVal, pnlPct: pnlP };
}
function calcO(o) {
  ...
  return { ...o, contratos: ct, primaCompra: pb, comision: com, subtotal: sub, ingresos: ing, estado, pnl: pnlVal, pnlPct: pnlP };
}
...
function showPage(page) {
  ...
  const titles = { dashboard:'Dashboard', capital:'Capital', posiciones:'Posiciones', trades:'Trades', 'plan-acciones':'Plan Acciones', 'plan-opciones':'Plan Opciones', socios:'Socios', watchlist:'Watchlist', importador:'Importar CSV Schwab', reportes:'Reportes' };
...
function getDash() {
  const c = cfg();
  const closedAcc = D.acciones.filter(a => a.estado === 'Cerrada');
  const closedOpc = D.opciones.filter(o => o.estado === 'Cerrada');
  ...
  const comm = [...D.acciones, ...D.opciones].reduce((s,x) => s + n(x.comision), 0);
...
function getPeriod(period) {
  ...
  const pA = D.acciones.filter(a => a.estado==='Cerrada' && ok(a.fechaCierre)).reduce((s,a) => s+n(a.pnl), 0);
  const pO = D.opciones.filter(o => o.estado==='Cerrada' && ok(o.fechaCierre)).reduce((s,o) => s+n(o.pnl), 0);
  ...
  return `
    ...
    <div class="plan-btn-grid">
      <div class="plan-big-btn stocks" onclick="showPage('plan-acciones')">
        ...
        <div class="plan-big-sub">Meta ${(cfg().metaAcc*100).toFixed(0)}% · Capital ${money(cfg().capAcc)}</div>
      </div>
      <div class="plan-big-btn options" onclick="showPage('plan-opciones')">
        <div class="plan-big-icon">🎯</div>
        <div class="plan-big-title">Plan del día</div>
        <div class="plan-big-meta">Opciones</div>
        <div class="plan-big-sub">Meta ${(cfg().metaOpc*100).toFixed(0)}% · Capital ${money(cfg().capOpc)}</div>
      </div>
    </div>
      ...
      <div class="tbl-wrap"><table>
        ...
        <tbody>
          ${openAcc.map(a => { const meta = targetExit(a, true); return `<tr>
            ...
            <td><button class="btn btn-sm" onclick="openEditTrade('acc','${esc(a.ticker)}')">✏</button></td>
          </tr>`; }).join('')}
          ${openOpc.map(o => { const meta = targetExit(o, false); return `<tr>
            ...
            <td><button class="btn btn-sm" onclick="openEditTrade('opc','${esc(o.empresa)}')">✏</button></td>
          </tr>`; }).join('')}
        </tbody>
      </table></div>`}
    </div>
    ...
    <div class="card">
      ...
      <div class="tbl-wrap"><table>
        ...
        <tbody>${closed10.map(t => `<tr>
          ...
          <td><button class="btn btn-sm" onclick="openEditTrade('${t.kind}','${esc(t.name)}')">✏</button></td>
        </tr>`).join('')}</tbody>
      </table></div>`}
    </div>`;
}
...
async function doCapital(tipo) {
  ...
  try {
    await api('saveCapital', { tipo, monto, socioMode: mode === 'nuevo' ? 'nuevo' : (mode === 'principal' ? 'existente' : 'existente'), socioRow: (mode !== 'principal' && mode !== 'nuevo') ? mode : '', socioNombre: socioNombre || 'Socio principal' });
    ...
    <div class="card">
      ${openAcc.length ? `<div class="tbl-wrap"><table>
        ...
        <tbody>${openAcc.map(a => { const m = targetExit(a,true); return `<tr>
          ...
          <td>${dateText(a.fechaCompra)}</td>
          <td><button class="btn btn-sm" onclick="openEditTrade('acc','${esc(a.ticker)}')">✏ Editar</button></td>
        </tr>`; }).join('')}</tbody>
      ...
      ${openOpc.length ? `<div class="tbl-wrap"><table>
        ...
        <tbody>${openOpc.map(o => {
          ...
          return `<tr>
            ...
            <td><button class="btn btn-sm" onclick="openEditTrade('opc','${esc(o.empresa)}')">✏ Editar</button></td>
          </tr>`;
        }).join('')}</tbody>
...
// ── Plan ─────────────────────────────────────────────────────
function buildMonthly(capital, dailyPct, meses, diasMes) {
  let cur = capital; const rows = [];
  for (let m = 1; m <= meses; m++) {
    const start = cur;
    cur = cur * Math.pow(1 + dailyPct / 100, diasMes);
    rows.push({ month: m, start, end: cur, gain: cur - start, pct: start ? (cur - start) / start : 0 });
  }
  return rows;
}
function buildDaily(capital, dailyPct, diasMes, month) {
  let cur = capital * Math.pow(1 + dailyPct / 100, (month - 1) * diasMes);
  const rows = [];
  const base = new Date(); base.setHours(0,0,0,0);
  base.setDate(base.getDate() + (month-1)*diasMes);
  for (let d = 1; d <= diasMes; d++) {
    const start = cur, target = start * (dailyPct / 100);
    cur = start + target;
    const dt = new Date(base); dt.setDate(base.getDate() + d - 1);
    rows.push({ day: d, date: dt.toISOString().slice(0,10), start, target, end: cur });
  }
  return rows;
}
...
function renderPlan(kind) {
  ...
  const c = cfg();
  const capCuenta = isAcc ? c.capAcc : c.capOpc;
  const uso = isAcc ? c.usoAcc : c.usoOpc;
  const capitalOp = capCuenta * uso;
  const dailyPct = (isAcc ? c.metaAcc : c.metaOpc) * 100;
  const diasMes = c.diasMes, meses = c.meses;
  const month = selectedPlanMonth[kind] || 1;
  const day   = Math.min(selectedPlanDay[kind] || 1, diasMes);
  const months = buildMonthly(capitalOp, dailyPct, meses, diasMes);
  const daily  = buildDaily(capitalOp, dailyPct, diasMes, month);
  const curr = daily[day - 1];
  const mPct = (Math.pow(1 + dailyPct/100, diasMes) - 1) * 100;
  const final12 = months[meses - 1]?.end || capitalOp;

  // Trades registrados de hoy para este plan
  const todayTrades = isAcc
    ? D.acciones.filter(a => a.fechaCompra === todayISO || a.fechaCierre === todayISO)
    : D.opciones.filter(o => o.fechaApertura === todayISO || o.fechaCierre === todayISO);
  const realPnlHoy = todayTrades.filter(t => t.estado === 'Cerrada').reduce((s,t) => s + n(t.pnl), 0);
  const progPct = curr.target > 0 ? Math.min((realPnlHoy / curr.target) * 100, 150) : 0;

  return `
    <div class="btn-row">
      <button class="btn ${isAcc?'btn-green':'btn-blue'}" onclick="openPlanPosition('${kind}')">+ Agregar trade del plan</button>
      <button class="btn btn-red" onclick="openModal('cerrar-pos')">✕ Cerrar posición</button>
      <button class="btn" onclick="openModal('config')">⚙ Editar reglas del plan</button>
    </div>

    <div class="kpi-grid">
      ${kpi('Capital de cuenta', money(capCuenta), mxn(capCuenta), isAcc?'teal':'blue')}
      ${kpi('Capital operable', money(capitalOp), `${(uso*100).toFixed(0)}% del capital`, isAcc?'teal':'blue')}
      ${kpi('Meta del trade', money(curr.target), `+${dailyPct.toFixed(1)}%`, isAcc?'teal':'blue')}
      ${kpi('Siguiente si cumples', money(curr.end), `Día ${day} de ${diasMes}`, isAcc?'teal':'blue')}
      ${kpi('% mínimo mensual', (mPct.toFixed(1))+'%', `${diasMes} días de trading`, 'amber')}
      ${kpi('Capital final 12m', money(final12), mxn(final12), isAcc?'teal':'blue')}
      ${kpi('P&L real hoy', money(realPnlHoy), mxn(realPnlHoy), realPnlHoy>=0?'teal':'red')}
      ${kpi('Estado hoy', progPct>=100?'Meta lograda ✓':'En progreso', `${progPct.toFixed(0)}% completado`, progPct>=100?'teal':'amber')}
    </div>

    <div class="sec-title">Progreso del día ${day} · Mes ${month}</div>
    <div class="card">
      <div style="display:flex;justify-content:space-between;font-size:13px;margin-bottom:8px">
        <span style="color:var(--muted)">Operación sugerida: <strong>${money(curr.start)}</strong></span>
        <span>Real: <strong class="${realPnlHoy>=0?'green':'red'}">${money(realPnlHoy)}</strong> / Meta: <strong>${money(curr.target)}</strong></span>
      </div>
      <div class="prog-bar"><div class="prog-fill" style="width:${Math.max(progPct,0)}%;background:${progPct>=100?(isAcc?'var(--teal)':'var(--blue)'):'var(--red)'}"></div></div>
      <div class="prog-label">${progPct>=100?'Meta alcanzada ✓':`Faltan: ${money(Math.max(curr.target-realPnlHoy,0))}`}</div>
    </div>

    <div class="sec-title">Trades de hoy</div>
    <div class="card">
      ${todayTrades.length === 0 ? `<p style="color:var(--muted);margin-bottom:10px">Sin trades registrados hoy.</p>` :
      `<div class="tbl-wrap"><table>
        <thead><tr><th>Activo</th><th>Estado</th><th>Inversión</th><th>P&L</th><th>P&L %</th><th>Siguiente operación</th><th></th></tr></thead>
        <tbody>${todayTrades.map(t => {
          const isClose = t.estado === 'Cerrada';
          const nextOp = isClose ? money(n(t.subtotal) + n(t.pnl)) : '—';
          return `<tr>
            <td><strong>${esc(isAcc ? t.ticker : t.empresa)}</strong></td>
            ...
            <td class="${isClose?(n(t.pnl)>=0?'green':'red'):''}"><strong>${nextOp}</strong></td>
            <td><button class="btn btn-sm" onclick="openEditTrade('${isAcc?'acc':'opc'}','${esc(isAcc?t.ticker:t.empresa)}')">✏</button></td>
          </tr>`;
        }).join('')}</tbody>
      </table></div>`}
      <button class="btn ${isAcc?'btn-green':'btn-blue'}" style="margin-top:10px" onclick="openPlanPosition('${kind}')">+ Registrar trade</button>
    </div>

    <div class="sec-title">Selecciona mes del plan</div>
    <div class="plan-month-grid">
      ${months.map(m => `<button class="month-card ${m.month===month?'active':''}" onclick="selectPlanMonth('${kind}',${m.month})">
        ...
        <span>${money(m.start)} → ${money(m.end)}</span>
        <small>+${money(m.gain)} (${(m.pct*100).toFixed(1)}%)</small>
      </button>`).join('')}
    </div>

    <div class="sec-title">Días del mes ${month}</div>
    <div class="card">
      <div class="tbl-wrap"><table class="plan-day-table">
        <thead><tr><th>Día</th><th>Fecha</th><th>Monto operación</th><th>Meta P&L</th><th>Capital siguiente</th><th>Capital MXN</th><th></th></tr></thead>
        <tbody>${daily.map(r => `<tr class="${r.day===day?'current-day':''}">
          <td>${r.day} / ${diasMes}</td>
          <td>${dateText(r.date)}</td>
          <td><strong>${money(r.start)}</strong></td>
          <td class="green">+${money(r.target)}</td>
          <td>${money(r.end)}</td>
          <td>${mxn(r.end)}</td>
          <td><button class="btn btn-sm" onclick="selectPlanDay('${kind}',${r.day})">Usar</button></td>
        </tr>`).join('')}</tbody>
      </table></div>
    </div>

    <div class="sec-title">Proyección 12 meses — USD y MXN</div>
    <div class="card">
...
function drawPlanChart(kind) {
  const isAcc = kind === 'acciones';
  const c = cfg();
  const capital = isAcc ? c.capAcc * c.usoAcc : c.capOpc * c.usoOpc;
  const dailyPct = (isAcc ? c.metaAcc : c.metaOpc) * 100;
  const months = buildMonthly(capital, dailyPct, c.meses, c.diasMes);
  ...
  chartInstances[kind] = new Chart(canvas, {
    ...
    data: {
      ...
      datasets: [
        { label: 'USD', data: months.map(m => +m.end.toFixed(2)), borderColor: isAcc ? '#1D9E75' : '#185FA5', backgroundColor: isAcc ? 'rgba(29,158,117,.1)' : 'rgba(24,95,165,.1)', borderWidth: 2.5, pointRadius: 4, fill: true, tension: 0.3 },
        { label: 'MXN', data: months.map(m => +(m.end * FX).toFixed(2)), borderColor: '#BA7517', backgroundColor: 'rgba(186,117,23,.05)', borderWidth: 1.5, borderDash: [5,4], pointRadius: 3, fill: false, tension: 0.3 }
      ]
    },
    options: {
      responsive: true, maintainAspectRatio: false,
      plugins: { legend: { position: 'top' }, tooltip: { callbacks: { label: ctx => ctx.datasetIndex === 0 ? 'USD: ' + money(ctx.parsed.y) : 'MXN: $' + ctx.parsed.y.toLocaleString('es-MX', {minimumFractionDigits:2}) } } },
...
// ── Importador CSV Schwab ─────────────────────────────────────
function renderImportador() {
  return `
    <div class="alert-info">
      <strong>Instrucciones Schwab:</strong><br>
      1. En Schwab → Accounts → History → selecciona rango de fechas → Export → CSV<br>
      2. Abre el CSV, <strong>borra columnas con datos personales</strong> (nombre, número de cuenta, dirección)<br>
      3. Deja solo: Date, Action, Symbol, Description, Quantity, Price, Fees &amp; Comm, Amount<br>
      4. Copia y pega el contenido abajo (sin encabezados, solo datos)
    </div>
    <div class="card">
      <div class="card-title">Pegar CSV de Schwab</div>
      <textarea class="csv-area" id="csv-input" placeholder="Pega aquí las filas del CSV de Schwab...&#10;Ejemplo:&#10;05/10/2025,Buy,AAPL,APPLE INC,10,175.00,0.65,-1750.65&#10;05/12/2025,Sell,AAPL,APPLE INC,10,182.00,0.65,1819.35"></textarea>
      <div class="btn-row" style="margin-top:10px">
        <button class="btn btn-green" onclick="parseSchwabCSV()">📥 Procesar CSV</button>
        <button class="btn" onclick="$('csv-input').value=''">Limpiar</button>
      </div>
    </div>
    <div id="csv-preview-wrap" class="hidden">
      <div class="sec-title">Vista previa — revisa antes de importar</div>
      <div class="card csv-preview" id="csv-preview"></div>
      <div class="btn-row">
        <button class="btn btn-green" onclick="importCSVTrades()">✓ Importar todos</button>
...
function parseSchwabCSV() {
  const raw = $('csv-input').value.trim();
  if (!raw) { showToast('Pega el contenido del CSV primero.', 'error'); return; }
  const lines = raw.split('\n').map(l => l.trim()).filter(Boolean);
  csvParsed = [];
  const errors = [];
  lines.forEach((line, idx) => {
    const cols = line.split(',').map(c => c.replace(/^"|"$/g,'').trim());
    if (cols.length < 6) { errors.push(`Línea ${idx+1}: columnas insuficientes`); return; }
    const [date, action, symbol, desc, qty, price, fees, amount] = cols;
    if (!symbol || symbol === 'Symbol') return;
    const act = (action||'').toLowerCase();
    if (act.includes('buy') || act.includes('sell') || act.includes('compra') || act.includes('venta')) {
      csvParsed.push({ date: date||'', action: act.includes('buy')||act.includes('compra') ? 'buy' : 'sell', symbol: symbol.toUpperCase(), desc: desc||'', qty: Math.abs(n(qty)), price: Math.abs(n(price)), fees: Math.abs(n(fees)), amount: n(amount), raw: line });
    }
  });
  if (csvParsed.length === 0) { showToast('No se encontraron operaciones de compra/venta.', 'error'); return; }
  const preview = $('csv-preview');
  preview.innerHTML = `<div class="tbl-wrap"><table>
    <thead><tr><th>#</th><th>Fecha</th><th>Acción</th><th>Ticker</th><th>Cantidad</th><th>Precio</th><th>Comisión</th><th>Monto</th></tr></thead>
    <tbody>${csvParsed.map((r,i) => `<tr>
      <td>${i+1}</td>
      <td>${esc(r.date)}</td>
      <td><span class="badge ${r.action==='buy'?'badge-green':'badge-red'}">${r.action==='buy'?'Compra':'Venta'}</span></td>
      <td><strong>${esc(r.symbol)}</strong></td>
      <td>${r.qty}</td>
      <td>${money(r.price)}</td>
      <td>${money(r.fees)}</td>
      <td class="${r.action==='sell'?'green':'red'}">${money(r.amount)}</td>
    </tr>`).join('')}</tbody>
  </table></div>
  ${errors.length ? `<div class="alert-warn" style="margin-top:10px">⚠️ ${errors.length} líneas ignoradas: ${errors.slice(0,3).join(', ')}</div>` : ''}
  <p style="font-size:12px;color:var(--muted);margin-top:10px">Se importarán <strong>${csvParsed.length}</strong> operaciones.</p>`;
...
async function importCSVTrades() {
  ...
  for (const r of csvParsed) {
    try {
      // Try to detect options by symbol format (e.g. AAPL 230120C00150000)
      const isOption = r.symbol.length > 6 || r.desc.toLowerCase().includes('call') || r.desc.toLowerCase().includes('put');
      if (r.action === 'buy') {
        if (isOption) {
          await api('saveOpcion', { empresa: r.symbol.slice(0,6).trim(), strike: 0, tipo: r.desc.toLowerCase().includes('put') ? 'PUT' : 'CALL', contratos: r.qty, primaCompra: r.price, comision: r.fees, fechaApertura: formatSchwabDate(r.date) });
        } else {
          await api('saveAccion', { ticker: r.symbol, acciones: r.qty, precioCompra: r.price, comision: r.fees, fechaCompra: formatSchwabDate(r.date) });
        }
      } else {
        if (isOption) {
          await api('closeOpcion', { empresa: r.symbol.slice(0,6).trim(), primaVenta: r.price, comisionCierre: r.fees, fechaCierre: formatSchwabDate(r.date) });
        } else {
          await api('closeAccion', { ticker: r.symbol, precioVenta: r.price, comisionCierre: r.fees, fechaCierre: formatSchwabDate(r.date) });
        }
      }
      ok++;
    } catch(e) { err++; }
...
function openModal(type) {
  ...
  if (type === 'config') {
    const c = cfg();
    box.innerHTML = `<div class="modal-title">⚙ Configuración del plan</div>
      <div class="form-grid">
        <div class="form-group"><label>Tu nombre</label><input type="text" id="m-cfg-nombre" value="${esc(c.nombre)}"></div>
        <div class="form-group"><label>Capital acciones ($)</label><input type="number" id="m-cfg-cap-acc" value="${c.capAcc}" step="0.01"></div>
        <div class="form-group"><label>Capital opciones ($)</label><input type="number" id="m-cfg-cap-opc" value="${c.capOpc}" step="0.01"></div>
        <div class="form-group"><label>Meta diaria acciones (%)</label><input type="number" id="m-cfg-meta-acc" value="${(c.metaAcc*100).toFixed(1)}" step="0.1"></div>
        <div class="form-group"><label>Meta diaria opciones (%)</label><input type="number" id="m-cfg-meta-opc" value="${(c.metaOpc*100).toFixed(1)}" step="0.1"></div>
      ...
      <div class="modal-footer">
        <button class="btn btn-green" onclick="saveConfig()">Guardar</button>
        <button class="btn" onclick="closeModal()">Cancelar</button>
      </div>`;
    return;
  }
  ...
  if (type === 'cerrar-pos') {
    const openAcc = D.acciones.filter(a => a.estado === 'Abierta').map(a => ({ kind:'acc', label:`${a.ticker} — ${a.acciones} acciones @ ${money(a.precioCompra)}`, key: a.ticker }));
    const openOpc = D.opciones.filter(o => o.estado === 'Abierta').map(o => ({ kind:'opc', label:`${o.empresa} ${o.tipo} $${o.strike} — ${o.contratos} ct`, key: o.empresa }));
...
function openPlanQuestion(kind) {
  const c = cfg();
  const target = kind === 'acciones' ? c.metaAcc : c.metaOpc;
  const box = $('modal-box');
  box.innerHTML = `<div class="modal-title">Nuevo trade de ${kind}</div>
    <p style="color:var(--muted);margin-bottom:1rem;line-height:1.5">¿Vas a continuar tu plan del <strong>${(target*100).toFixed(1)}%</strong>?</p>
    <div class="modal-footer">
      <button class="btn ${kind==='acciones'?'btn-green':'btn-blue'}" onclick="closeModal();showPage('${kind==='acciones'?'plan-acciones':'plan-opciones'}');openPlanPosition('${kind}')">Sí, continuar el plan</button>
      <button class="btn" onclick="closeModal();openModal('nueva-pos-${kind==='acciones'?'accion':'opcion'}')">No, registrar sin plan</button>
    </div>`;
  $('modal-overlay').classList.remove('hidden');
}
...
function openEditTrade(kind, key) {
  const item = kind === 'acc'
    ? D.acciones.find(a => a.ticker === key)
    : D.opciones.find(o => o.empresa === key);
  if (!item) { showToast('No se encontró la operación.', 'error'); return; }
  const box = $('modal-box');
  if (kind === 'acc') {
    box.innerHTML = `<div class="modal-title">✏ Editar acción — ${esc(item.ticker)}</div>
      ...
      <div class="modal-footer">
        <button class="btn btn-green" onclick="doEditAccion('${esc(item.ticker)}')">Guardar cambios</button>
        <button class="btn" onclick="closeModal()">Cancelar</button>
      </div>`;
  } else {
    box.innerHTML = `<div class="modal-title">✏ Editar opción — ${esc(item.empresa)}</div>
      ...
      <div class="modal-footer">
        <button class="btn btn-green" onclick="doEditOpcion('${esc(item.empresa)}')">Guardar cambios</button>
        <button class="btn" onclick="closeModal()">Cancelar</button>
      </div>`;
  }
  $('modal-overlay').classList.remove('hidden');
...
async function doNuevaAccion() {
  ...
  try {
    await api('saveAccion', { ticker, acciones, precioCompra: precio, comision, fechaCompra: fecha });
    closeModal(); await syncAll();
...
async function doNuevaOpcion() {
  ...
  try {
    await api('saveOpcion', { empresa, strike, tipo, contratos, primaCompra: prima, comision, fechaApertura: fecha, fechaVencim: venc });
    closeModal(); await syncAll();
    showToast('Opción registrada.', 'success');
  } catch(e) { showToast(e.message, 'error'); }
}
...
async function doCerrarPos(all) {
  ...
  try {
    let result;
    if (pos.kind === 'acc') result = await api('closeAccion', { ticker: pos.key, precioVenta: price, comisionCierre: comm, fechaCierre: date });
    else result = await api('closeOpcion', { empresa: pos.key, primaVenta: price, comisionCierre: comm, fechaCierre: date });
...
async function doEditAccion(originalTicker) {
  const payload = {
    originalTicker,
...
async function doEditOpcion(originalEmpresa) {
  const payload = {
    originalEmpresa,
...
function loadDemoData() {
  D.config = { nombre:'Luis', capAcciones:1000, capitalAcciones:1000, capOpciones:500, capitalOpciones:500, metaAcciones:0.03, metaOpciones:0.20, meses:12, tipoCambio:17.5, diasMes:20, usoCapitalAcciones:1, usoCapitalOpciones:0.33 };
  FX = 17.5;
  D.acciones = [
